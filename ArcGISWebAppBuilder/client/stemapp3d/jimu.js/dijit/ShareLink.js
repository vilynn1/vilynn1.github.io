  ...
  function(declare, _WidgetBase, _TemplatedMixin, _WidgetsInTemplateMixin, lang, array, dojoClass, html,
    ...
    var so = declare([_WidgetBase, _TemplatedMixin, _WidgetsInTemplateMixin], {
      ...
      share: {
        shareEmailSubject: "",
        shareTwitterTxt: "",
        languages: ["ar", "bg", "bs", "cs", "da", "de", "en", "el", "es", "et", "fi", "fr", "he", "hr", "hu",
      ...
      _initMap: function() {
        ...
        if (window._widgetManager.sceneView) {
          this.map = window._widgetManager.sceneView;
      ...
      _mapEventUpdateUrls: function() {
        if (this.map) {
          ...
          // })));
          this.own(this.map.watch("extent", lang.hitch(this, this._mapEventHandler)));
        }
      },
      //NO MapUrlParamsHandler.js in 3D, so do not update url
      ...
      _initUI: function() {
        ...
        if (!this._isShowBackBtn) {
          ...
          dojoClass.add(this.linkOptions, "displaynone");
        } else {
      ...
      _updateResUrls: function(param) {
        ...
        if (this.optionSrc === "currentMapExtent") {
          ...
          this.resultUrl = this.baseHrefUrl;
      ...
      _fixUrlIfIsOnline: function(url) {
        ...
        if (this._isOnline && this._isSharedToPublic) {
          ...
          // }
          var pathname = "/apps/webappviewer3d/index.html";//2d:webappviewer, 3d:webappviewer3d
      ...
      shortenUrl: function(url, bitlyUrl) {
        ...
        esriRequest(uri, {
          ...
          if (result && result.data && result.data.status_code === 200 &&
            result.data.data && result.data.data.url && result.data.data.url.length > 0) {
            def.resolve(result.data.data.url);
          } else {
            def.reject(result);
          }
      ...
      _query: function(/*outFields, url, map*/) {
        ...
        return queryTask.executeQueryJSON(url, queryParams);*/
      },
