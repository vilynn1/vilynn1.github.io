...
function(lang, array, html, has, config, ioQuery, query, nlt, Deferred, all, on, json, cookie,
  dojoNumber, dateLocale, nlsBundle, base64, esriNS, esriConfig,/* arcgisUtils,*/
  // PopupTemplate,
  SpatialReference, Extent, Multipoint, Polyline, Polygon, webMercatorUtils, geometryService,
  ProjectParameters, esriUrlUtils, esriRequest,
  /*graphicsUtils,*/ sharedUtils
) {
  /* global dojoConfig, ActiveXObject */
  var mo = {};
  ...
  mo.restoreToDefaultWebMapExtent = function(map, webMapResponse, geoServiceUrl) {
    ...
    if (mapWkid === 4326) {
      ...
      if (map.spatialReference.isWebMercator) {
        ...
        var url = null;
        if (esriConfig && esriConfig.defaults && esriConfig.defaults.geometryServiceUrl && typeof esriConfig.defaults.geometryServiceUrl === 'string' && lang.trim(esriConfig.defaults.geometryServiceUrl)) {
          url = esriConfig.defaults.geometryServiceUrl
        } else if (geoServiceUrl && typeof geoServiceUrl === 'string' && lang.trim(geoServiceUrl)) {
          url = geoServiceUrl
        } else {
          url = mo.getServices() && mo.getServices().geometryService
        }
        if (!url) {
          return;
        }
        ...
        url = lang.trim(url);
        geometryService.project(url, params).then(function(geometries) {
  ...
  */
  ...
  mo.fieldFormatter = {
    ...
    getFormattedDate: function(/*timeNumber, format*/ str) {
    ...
    getCodedValue: function(domain, v) {
      if (domain && domain.codedValues) {
        for (var i = 0, len = domain.codedValues.length; i < len; i++) {
          ...
          if (mo.isDefined(v) && lang.exists('code', cv) &&
  ...
  */
  ...
  mo.deleteMapOptions = function(mapOptions){
    ...
    delete mapOptions.initialState;
  };
  ...
  mo.isVersion4API = function(){
    return esriNS.version.indexOf("4") === 0;
  };

  mo.getEsriConfig = function(){
    return esriConfig;
  };

  mo.getEsriConfigRequestObject = function(){
    var request = null;
    if (mo.isVersion4API()) {
      //for 4.x API
      request = esriConfig.request;
    } else {
      //for 3.x API
      request = esriConfig.defaults.io;
    }
    return request;
  };

  mo.setGlobalGeometryService = function(url){
    if(mo.isVersion4API()){
      esriConfig.geometryServiceUrl = url;
    }else{
      esriConfig.defaults.geometryServiceUrl = url;
    }
    return url;
  };

  mo.getQueryableFeatureLayersFromMap = function(/* API 4.0 */ map){
    var layers = [];
    if(map.layers){
      layers = map.layers.filter(function(layer){
        return layer.declaredClass === "esri.layers.FeatureLayer" && layer.url &&
         layer.supportedQueryFormats &&
         layer.supportedQueryFormats.toUpperCase().indexOf("JSON") >= 0;
      });
    }
    return layers;
  };
  ...
  mo.getAppHref = function(){
    var href = "";
    var isBuilder = mo.isInConfigOrPreviewWindow();
    if(isBuilder){
      ...
      try {
        ...
        if (fakeContent) {
          ...
          if (contentImgs && contentImgs.length) {
            for (var i = 0, len = contentImgs.length; i < len; i++) {
              var img = contentImgs[i];
              html.setStyle(img, 'maxWidth', (contentWidth - scrollerWidth) + 'px');
            }
          }
