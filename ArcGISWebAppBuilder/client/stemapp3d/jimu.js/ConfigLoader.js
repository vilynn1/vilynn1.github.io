...
function (declare, lang, array, html, dojoConfig, cookie,
  Deferred, all, xhr, jimuUtils, privilegeUtils, WidgetManager, sharedUtils, tokenUtils,
  ...
  clazz = declare(null, {
    ...
    ********************************************************/
      ...
      return this._tryLoadConfig().then(lang.hitch(this, function(appConfig) {
        ...
        if(this.urlParams.id){
          return this.loadWidgetsManifest(appConfig).then(lang.hitch(this, function() {
            ...
            if(appConfig.title){
              if(window.isBuilder){
                document.title = jimuUtils.stripHTML(appConfig.title) + ' - ArcGIS Web AppBuilder';
              }else{
          ...
          return this._proesssWebTierAndSignin(appConfig).then(lang.hitch(this, function() {
            ...
            if(appConfig.map.itemId){
              ...
              return portalUtils.getDefaultWebScene(appConfig.portalUrl).then(function(itemId){
    ...
    processResourceInAppConfigForConfigLoader:function(appConfig, urlParams){
      ...
      //1.Traverse appConfig, get all the resources URL, replace its ${itemId} to app2Id
    ...
    processProxy: function(appConfig){
      ...
      io.alwaysUseProxy = appConfig.httpProxy &&
                          appConfig.httpProxy.useProxy &&
                          appConfig.httpProxy.alwaysUseProxy;
      io.proxyUrl = "";
      io.proxyRules = [];

      if (appConfig.httpProxy && appConfig.httpProxy.useProxy && appConfig.httpProxy.url) {
        io.proxyUrl = appConfig.httpProxy.url;
      }
    ...
    _processSignIn: function(portalUrl, isWebTier){
      ...
      if(window.appInfo.isRunInPortal){
        ...
        defSignIn.catch(function() {}).then(lang.hitch(this, function(){ //jshint ignore:line
          def.resolve();
        }));
      }else{
