...
function (declare, lang, array, html, topic, Deferred, on, jimuUtils, WidgetManager,
  AppVersionManager, ConfigLoader, tokenUtils, portalUrlUtils, portalUtils, esriConfig) {
  ...
  clazz = declare(null, {
    ...
    listenBuilderEvents: function(){
      ...
      topic.subscribe('builder/sceneViewChanged', lang.hitch(this, this._onSceneViewChanged));
    ...
    _showErrorMessage: function(error){
      if(error.isSelfOrigin === false) {
        ...
        var str = "Unable to create map: " + (error.message || error);
        html.create('div', {
          'class': 'app-error',
          innerHTML: jimuUtils.sanitizeHTML(str)
        }, document.body);
      }
    },
    ...
    _onSceneViewChanged: function(_newJson){
      // transfer obj to another iframe may cause problems on IE8
      var newJson = jimuUtils.reCreateObject(_newJson);
    ...
    ************************************************/
    ...
    _addDefaultGeometryService: function(appConfig){
      ...
      esriConfig.geometryServiceUrl = geoServiceUrl;
      jimuUtils.setGlobalGeometryService(appConfig.geometryService);
    },
      ...
      jimuUtils.visitElement(config, lang.hitch(this, function(e, info){
        ...
        if(e.widgets){
          ...
          if(e.widgets.length === 1){
            ...
            e.label = e.label? e.label: window.jimuNls.common.group + '_' + info.index;
          }
        }else{
          e.gid = info.groupId;
          if(e.uri){
