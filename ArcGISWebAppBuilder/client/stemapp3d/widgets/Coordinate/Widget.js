...
define([
  ...
  function(declare, html, _WidgetsInTemplateMixin, Point, SpatialReference, BaseWidget, utils, Message,
    ...
    var clazz = declare([BaseWidget, _WidgetsInTemplateMixin], {
      ...
      initPopMenu: function(spatialReferences) {
        ...
        if (Object.prototype.toString.call(spatialReferences) !== "[object Array]") {
          ...
          this.addMenuItem(
            ...
            spatialReferences.elevationUnit,
            spatialReferences.eyeAltUnit,
            null,
            null,
      ...
      _addAllMenuItems: function() {
        ...
        for (var i = 0; i < len; i++) {
          this.addMenuItem(
            ...
            this.config.spatialReferences[i].elevationUnit,
            this.config.spatialReferences[i].eyeAltUnit,
      ...
      onLocateButtonClick: function() {
        ...
        if (this.canShowInClient(this.selectedWkid)) {
          if (this.enableRealtime) {
            ...
            this._setCoordinateInnerHTML(this.nls.hintMessage);
      ...
      _getEyeInfoUnitRate: function(elevationUnit){
        var unit = ("english" === elevationUnit ? "FOOT" : "METER");//default is METER
        return unitUtils.getUnitRate("METER", unit);
      },

      addMenuItem: function(name, wkid, outputUnit, elevationUnit, eyeAltUnit, tfWkid, forward, _options) {
        ...
        var item = {
          ...
          tfWkid: tfWkid || "",
          options: _options,
          elevationUnitRate: this._getEyeInfoUnitRate(elevationUnit),
          eyeAltUnitRate: this._getEyeInfoUnitRate(eyeAltUnit)
        };
        if (item.tfWkid) {
      ...
      _projectMapPoint: function(point) {
        ...
        params.outSpatialReference = new SpatialReference(parseInt(outWkid, 10));//for 4.x api ,#15905

        //this.coordinateInfo.innerHTML = this.nls.computing;
        this._setCoordinateInnerHTML(this.nls.computing);
      ...
      _displayOnClient: function(mapPoint) {
        ...
        if (convertInClient) {
          ...
          if (options.defaultUnit === outUnit) {
            ...
            this._setCoordinateInnerHTML(this.coordinateInfo.innerHTML += " " + this._unitToNls(outUnit));
            return;
          }
      ...
      _displayUsngOrMgrs: function(outUnit, y, x) {
        if ("MGRS" === outUnit) {
          ...
          this._setCoordinateInnerHTML(usng.LLtoMGRS(y, x, 5));
        } else if ("USNG" === outUnit) {
          //this.coordinateInfo.innerHTML = usng.LLtoUSNG(y, x, 5);
          this._setCoordinateInnerHTML(usng.LLtoUSNG(y, x, 5));
        }
      ...
      _displayCoordinatesByOrder: function(x, y) {
        ...
        if (displayOrderLonLat) {
          ...
          this._setCoordinateInnerHTML(x + "  " + y);
        } else {
          //this.coordinateInfo.innerHTML = y + "  " + x;
          this._setCoordinateInnerHTML(y + "  " + x);
        }
      },
      ...
      _setCoordinateInfo: function(isJustElev) {
        if (this._cache._mouseX && this._cache._mouseY) {
          this.sceneView.hitTest({
            x: this._cache._mouseX,
            y: this._cache._mouseY
          }).then(lang.hitch(this, function (position) {
            if (!isJustElev) {
      ...
      _setLonLat: function(position) {
        if (typeof position !== "undefined" && position.ground && position.ground.mapPoint !== null &&
          ((position.ground.mapPoint.latitude !== null && position.ground.mapPoint.longitude !== null) ||
          (position.ground.mapPoint.x !== null && position.ground.mapPoint.y !== null))
        ) {
          //with lat/lon or with x/y
          this._displayOnClient(position.ground.mapPoint);
        } else {
      ...
      _setElev: function(position) {
        if (position.results && position.results[0] &&
          ...
          this._setElevInfo(position.results[0].mapPoint);//result elev(raycasting)
          ...
          this._setElevInfo(position.ground.mapPoint);//ground elev
        } else {
          this._setElevInfo();
        }
      },
      ...
      _setElevInfo: function(pos) {
        ...
        this.elevInfo.innerHTML = utils.sanitizeHTML(elev);
      },
      _setEyeInfo: function() {
        ...
        this.eyeInfo.innerHTML = utils.sanitizeHTML(this.nls.EYE_ALT + " " + eyeAlt);
      },
      _trunNumToKmOrM: function(num, isElev) {
        ...
        if (isElev) {
          threshold = 10000;//switch to km if more than 10,000 m.
        }
      ...
      _setCoordinateInnerHTML: function (str) {
        var labelDom = this.coordinateInfo;
        if (labelDom) {
          labelDom.innerHTML = utils.sanitizeHTML(str);
        }
      }
    });
