...
function(declare, Color, html, lang, array, /*xhr,*/
  ...
  return declare([BaseWidget, _WidgetsInTemplateMixin], {
    ...
    startup: function() {
      ...
      def.then(lang.hitch(this, function(color) {
        if (color) {
          this.config.color = color;
        }

        if (!this.config.vizLayer || this.config.vizFields.length === 0) {
          this.panelMsgBlock.innerHTML = utils.sanitizeHTML("<p align='center'>" + this.nls.viz_error + "</p>");
        } else {
    ...
    _styleSync: function() {
      ...
      def.then(lang.hitch(this, function(color) {
        ...
        if (this.vizCards) {
          domStyle.set(this.vizCards.contentNode, "color", VizCards.CONTENT_COLOR/*color*/);
        }
        this.config.color = color;
    ...
    _getStyleColor: function(styleName) {
      return utils.getStyleColorInTheme(styleName);
    },
    ...
    _processViz: function(first) {
      ...
      this.titleNode.innerHTML = utils.sanitizeHTML(alias);
