  ...
  return declare([BaseResultRenderer, _TemplatedMixin], {
    ...
    postCreate: function(){
      ...
      if(this.value.features && this.value.features.length > 0){
        ...
        this._addResultURL(this.param, this.value.url);
      }else{
    ...
    _displayText: function(hideExportNode){
    ...
    _addResultURL: function(param, url) {
      this.resultLayer =  new FeatureLayer(url, {
      ...
      var renderer;
      if(param.renderer){
        renderer = rendererUtils.fromJson(param.renderer);
        if (param.renderer._showOthersSymbol === false) {
      ...
      this.own(on(this.resultLayer, 'load', lang.hitch(this, function(res) {
        if (res.layer && res.layer.fullExtent) {
          this.map.setExtent(res.layer.fullExtent.expand(1.2));
        }
      })));
    },
    ...
    _drawResultFeature: function(param, featureset){
      ...
      try{
        if (featureset.features && featureset.features.length > 0 &&
          ...
          if(extent){
            this.resultLayer.fullExtent = extent;
          }
        }
      }
      catch(e){
