...
function(declare, lang, array, html, on, keys, Tooltip, _WidgetBase, utils) {
  ...
  return declare(_WidgetBase, {
    ...
    postCreate: function(){
      ...
      this.state = 'closed';
//Added RJS
      if(this.widgetConfig.name === 'LayerToggleButton'){
        if(this.widgetConfig.selected){
          html.addClass(this.domNode, 'jimu-state-selected');
          html.setAttr(this.domNode, 'title', this.widgetConfig.label + ': Off');
        }else{
          html.setAttr(this.domNode, 'title', this.widgetConfig.label + ': On');
        }
      }
//End add RJS      
    },
