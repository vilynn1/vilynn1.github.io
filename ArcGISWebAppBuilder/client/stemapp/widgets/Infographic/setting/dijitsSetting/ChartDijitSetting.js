...
define([
  ...
  var clazz = declare([BaseDijitSetting, _WidgetsInTemplateMixin, Evented], {
    ...
    _setConfig: function(config) {
      ...
      var displayRange = 'AUTO';
      if (display) {
        backgroundColor = display.backgroundColor;
        displayRange = display.displayRange || 'AUTO';
      ...
      this.autoRangeRadio.setChecked(displayRange === 'AUTO');
      this.allRangeRadio.setChecked(displayRange === 'ALL');
    ...
    isValid: function() {
      ...
      return this.maxLabels.isValid() && this.hollowSizeControl.isValid() &&
        ...
        this.vAxisMin.isValid() && this.vAxisMax.isValid() && 
        this.hAxisMin.isValid() && this.hAxisMax.isValid();
    },
    ...
    getConfig: function(check) {
      ...
      //displayRange
      var autoRange = this.autoRangeRadio.checked;
      enumValues.displayRange = autoRange ? 'AUTO' : 'ALL';
      //tooltip
      if(type !== 'pie'){
      ...
      if (xName) {
        xAxis.name = jimuUtils.stripHTML(xName);
        xAxis.nameTextStyle = {};
      ...
      if (yName) {
        yAxis.name = jimuUtils.stripHTML(yName);
        yAxis.nameTextStyle = {};
