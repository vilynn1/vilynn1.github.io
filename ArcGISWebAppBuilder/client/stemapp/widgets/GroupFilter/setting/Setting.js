...
define([
  ...
  function(declare, BaseWidgetSetting, _WidgetsInTemplateMixin, SimpleTable, dom, domConstruct,
    ...
    Editor, utils, LayerInfos, Message, Popup, entities, LayersHandler, presetValuePicker, EditorXssFilter) {
    return declare([BaseWidgetSetting, _WidgetsInTemplateMixin], {
      ...
      setConfig: function(config) {
        ...
        this.chkHideDropdown.set('checked', this.config.hideHeaderSelection);
      ...
      getConfig: function() {
        if(this.layerList === null) {
          ...
          if(validGroups && validGroupsNames && validDuplicates && validTables) {
            ...
            this.config.headerText = this.editorXssFilter.sanitize(this._editorObj.get("value"));
            this.config.hideHeaderSelection = this.chkHideDropdown.checked;
            this.config.groups = [];
      ...
      createHeaderEditor: function() {
        ...
        if(this.config.hasOwnProperty("headerText")) {
          this._editorObj.set("value", (this.config.hasOwnProperty("headerText"))?this.config.headerText:this.nls.inputs.selectGroup);
        } else {
          this._editorObj.set("value", this.nls.inputs.selectGroup);
        }
      },
      ...
      */
      ...
      presetPickerPopup: function(pInput) {
        ...
        var filterPopup = new Popup({
          titleLabel: this.nls.popupHeader.label,
          width: 500,
          height: 420,
          content: valuePicker,
          buttons: [{
