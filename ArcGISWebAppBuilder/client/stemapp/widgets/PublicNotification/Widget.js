    ...
    return declare([BaseWidget, _WidgetsInTemplateMixin, Evented], {
      ...
      _setTheme: function () {
        ...
        if (this.appConfig.theme.name === "DartTheme") {
          jimuUtils.loadStyleLink(this.baseClass+'dartPanelOverrideCSS', this.folderUrl + "/css/dartPanelTheme.css", null);
        }
        else {
          styleLink = document.getElementById(this.baseClass+"dartPanelOverrideCSS");
          if (styleLink) {
       ...
       */
    _getExpressionByLayerURL: function (layerURL, addrLayerId) {
        var expr = "";
        for (var layerId in this.map._layers) {
        if (this.map._layers[layerId].url === layerURL && layerId === addrLayerId) {
      ...
      _updateAddresseesFromBufferGeometry: function () {
        ...
        if (this._bufferGeometry) {
          ...
          var layerId = this.addresseeSelect._getSelectedOptionsAttr().id;
          //get layer expression from layer
          var layerExp =
            this._getExpressionByLayerURL(this._addresseeSources[this.addresseeSelect.value].url, layerId);
