...
define([
  ...
  return declare([_WidgetBase, Evented], {
    ...
    postCreate: function () {
      ...
      this._allContentPanes = []; //to hold instance of all content panes for info or feature & related
    ...
    /**
     * Updates the height of list based on height of search container.
     * This function is added to adjust height, as after making Search text configurable
     * the floating icons where getting offscreen.
     */
    updateListHeight: function () {
      var searchOuterHeight = domStyle.get(this.searchOuterContainer, "height");
      searchOuterHeight += 23;
      domStyle.set(this.filterListMainDiv, "height", "calc(100% - " + searchOuterHeight + "px)");
      domStyle.set(this.domNode, "height", "calc(100% - " + searchOuterHeight + "px)");
    },
    ...
    **/
    _createPanels: function () {
      var templateDiv, proximityParent, proximityOptions;
      ...
      if (this.config.enableDirection) {
        ...
        this.own(this._tabContainer.on("tabChanged", lang.hitch(this, function (selectedTab) {
          ...
          if (selectedTab === this.nls.directionTabTitle) {
            //If selected tab is direction selected
            //if route is not calculted get directions
            //else if calculated set the last focus node in direction panel
            if (!this._routeCalculated) {
              this._initializeDirectionWidget();
            } else {
              if (domStyle.get(this.proximityButton.parentElement, "display") === "block") {
                jimuUtils.initLastFocusNode(this.domNodeObj, this.proximityButton);
              } else {
                this.emit("setLastNodeInFeatureInfo",
                  jimuUtils.getFocusNodesInDom(this._directionsWidget.domNode).reverse()[0]);
              }
            }
          } else {
            //If selected tab is iinfo tab update the last focusnode in info panle
      ...
      this.proximityButton = domConstruct.create("div", {
        "class": "esriCTProximityIconContainer",
        "title": this.nls.proximityButtonTooltip,
        "aria-label": this.nls.proximityButtonTooltip,
        "tabindex": "0",
        "role": "button"
      }, proximityParent);
      ...
      this.own(on(this.proximityButton, "click", lang.hitch(this, function () {
      ...
      this.own(on(this.proximityButton, 'keydown', lang.hitch(this, function (evt) {
      ...
      if (divItemTitle && divBackButton) {
        ...
        this.own(on(divItemTitle, "click, keypress", lang.hitch(this, function (event) {
          ...
          if (event.type === "keypress") {
            ...
            if (charOrCode !== keys.ENTER && charOrCode !== keys.SPACE) {
              return;
            }
          }
    ...
    **/
    displayLayerList: function (searchedLocation, serviceArea, attributeSearchList) {
      ...
      All(featureDeferArr).then(lang.hitch(this, function () {
        //Set class to last item in the list to identify the node for setting last focus node
        var validLayersList = query(".esriCTItemlList[tabindex='0']", this._panels.layerListPanel);
        if (validLayersList && validLayersList.length > 0) {
          //update current panel name
          this._currentPanelName = "layerListPanel";
          var lastElement = validLayersList[validLayersList.length - 1];
          domClass.add(lastElement, "esriCTLastLayerFocusNode");
        }
        this._onFeatureCountComplete();
        //Set last focus node of layer list panel once it is displayed
        this.emit("setLastNode", "layerListPanel");
      }));
    },
    ...
    * @memberOf widgets/NearMe/item-list
    **/
    _setItemName: function (templateDiv, value, itemClass) {
      if (itemClass) {
        templateDiv = query(itemClass, templateDiv)[0];
      }
      var divItemName = query(".esriCTItemName", templateDiv)[0];
      if (divItemName) {
        domAttr.set(divItemName, "innerHTML", value);
        domAttr.set(templateDiv, "aria-label", jimuUtils.stripHTML(value || ""));
        domAttr.set(templateDiv, "title", jimuUtils.stripHTML(value || ""));
      }
    },
    ...
    **/
    _displayFeatureList: function (item, defer) {
      ...
      this._setItemName(this._panels.featureListPanel, this._selectedLayer.title, ".esriCTPanelHeader");
    ...
    **/
    _setItemCount: function (templateDiv, value, isFeatureCount) {
      var divFeatureCount, divItemName, formattedValue, updatedTitle;
      ...
      if (divFeatureCount) {
        ...
        if (isFeatureCount) {
          ...
          if (this.config.showFeaturesCount) {
            ...
            //Also add feature count to the tooltip and aria-label of layer list item
            updatedTitle = domAttr.get(templateDiv, "title") + " " + number.format(value);
            domAttr.set(templateDiv, "title", updatedTitle);
            domAttr.set(templateDiv, "aria-label", updatedTitle);
          }
          if (value) {
            this._isNoFeature = false;
            //do not enable node if respective layer has no feature
            domClass.remove(templateDiv, "esriCTDisabled");
            //once feature count is shown it means layer is ready for accessing it features
            //so set the tabindex to allow it access by keys
            domAttr.set(templateDiv, "tabindex", "0");
            domAttr.set(templateDiv, "role", "button");
          ...
          //Also add distance info to the tooltip and aria-label of feature list item
          if (templateDiv) {
            updatedTitle = domAttr.get(templateDiv, "title") + " " + formattedValue;
            domAttr.set(templateDiv, "title", updatedTitle);
            domAttr.set(templateDiv, "aria-label", updatedTitle);
          }
        }
    ...
    **/
    _creatFeatureList: function (features) {
      ...
      var templateDiv = query(".esriCTPanelHeader", this._panels.featureListPanel)[0];
      ...
      if (features.length > 0) {
        features = this._getSortedFeatureList(features);
        if (templateDiv) {
          this._setItemCount(templateDiv, features.length, true);
        }
      }
      ...
      if (maxFeatureLength === 1) {
        ...
        for (i = 0; i < maxFeatureLength; i++) {
          ...
          if (i === maxFeatureLength - 1) {
            domClass.add(featureDiv, "esriCTLastFeatureFocusNode");
          }
          //set tabindex 0 so that rows can be accessed by keys
          domAttr.set(featureDiv, "tabindex", "0");
          domAttr.set(featureDiv, "role", "button");
          domAttr.set(featureDiv, "aria-label", features[i].getTitle());
          this._featureListContent.appendChild(featureDiv);
          this._setItemName(featureDiv, features[i].getTitle() || "");
          /**
           * -If showing attribute search result Or feature not having geometry Or
           ...
           */
          if (this._attributeSearchList || !features[i].geometry ||
            (features[i].geometry && features[i].geometry.type === "polygon" &&
              this.config.intersectSearchedLocation)) {
        ...
        focusUtil.focus(query(".esriCTItemlList[tabindex='0']", this._featureListContent)[0]);
        this.loading.hide();
        //Directly display feature Info Panel if only 1 feature found
        //By clicking on feature div
        if (maxFeatureLength === 1) {
          featureDiv.click();
        }
      }
    ...
    _getSortedFeatureList: function (features) {
      ...
      for (i = 0; i < features.length; i++) {
        ...
        if (this._attributeSearchList) {
          ...
          if (getGeodesicDistances && features[i].geometry) {
            features[i].distanceToLocation = this._getGeodesicDistances(features[i].geometry);
          } else {
            if (this._selectedPoint && features[i].geometry) {
    ...
    **/
    _showFeatureDetails: function (featureDiv, selectedFeature) {
      ...
      this._setItemName(this._panels.infoPanel, this._selectedLayer.title, ".esriCTPanelHeader");
      this._showPanel("infoPanel");
      var proximityParent = query(".esriCTProximityParent", this.domNode);
      if (proximityParent && proximityParent.length > 0) {
        proximityParent = proximityParent[0];
        //if selected feature not having geometry hide the proximity button if available
        if (!selectedFeature.geometry) {
          domClass.add(proximityParent, "esriCTHidden");
        } else {
          domClass.remove(proximityParent, "esriCTHidden");
        }
      }
      if (this._tabContainer) {
        //open information tab
        this._tabContainer.selectTab(this.nls.informationTabTitle);
        //if selected feature not having geometry hide tabs and directly show info only
        if (!selectedFeature.geometry || this._attributeSearchList) {
          domStyle.set(this._tabContainer.controlNode, "display", "none");
          domClass.add(this._tabContainer.containerNode, "esriCTTopBorder");
        } else {
          domStyle.set(this._tabContainer.controlNode, "display", "block");
    ...
    /**
     * Zooms to the selected feature using the zoomscale configured in searchSource settings
     * @param {Object} selectedFeature
     * @memberOf widgets/NearMe/item-list
     */
    _zoomToSelectedPointFeature: function (selectedFeature) {
      var extent;
      //we can get the zoom scale for searchSource settings if it is configured
      if (this.config.searchSourceSettings && this.config.searchSourceSettings.sources &&
        this.config.searchSourceSettings.sources.length > 0) {
        //loop through all the sources and get the zoom scale of seleted layer
        array.some(this.config.searchSourceSettings.sources,
          lang.hitch(this, function (searchSource) {
            //if layer id matches use its zoom scale and calculate extent to be set
            if (searchSource && searchSource.layerId && this._selectedLayer.id &&
              searchSource.layerId === this._selectedLayer.id) {
              extent = scaleUtils.getExtentForScale(
                this.map, searchSource.zoomScale).centerAt(selectedFeature.geometry);
              return true;
            }
          }));
      }
      //if extent is calulated using zoom scale use it else
      //by default zoom and center to the map's max zoom scale -1
      if (extent) {
    ...
    **/
    _displayFeatureInfo: function (selectedFeature) {
      var selectedFeaturePoint, selectedPoint, polyline, polylineJson, attributeSymbology;
      ...
      if (this._featureInfoPanel) {
        ...
        //- If selected feature dont have geometry then also dont show show approximate distances
        if (!this._attributeSearchList && selectedFeature.geometry &&
          !(selectedFeature.geometry && selectedFeature.geometry.type === "polygon" &&
            this.config.intersectSearchedLocation)) {
        ...
        if (this.config.attributeSymbology) {
          //get the configured symbology for selected layer and render attribute symbol
          attributeSymbology = this._getConfiguredSymbologyForLayer(this._selectedLayer.id);
          if (attributeSymbology && attributeSymbology.length > 0) {
            this._checkAttributeSymbology(selectedFeature, attributeSymbology);
          }
        }
      }
      ...
      if (this.config.zoomToFeature) {
        ...
        if (selectedFeature.geometry && selectedFeature.geometry.type === "point") {
          ...
          if (this._selectedPoint) {
            ...
            this._zoomToSelectedPointFeature(selectedFeature);
          }
    ...
    **/
    _showPopupInfo: function (feature, layer) {
      ...
      if (this._featureInfoPanel && feature) {
        //_layer and _map are the required properties to calculate values using Arcade expression
        //add them if not available in feature before getContent()
        if (!feature._layer) {
          feature._layer = layer;
        }
        if (!feature._layer._map) {
          feature._layer._map = this.map;
        }
        ...
        //when the poprender completes content-update set the last focus node of info panle
    ...
    **/
    _showAttachments: function (feature, attachmentContainer, layer, popupRenderer) {
      ...
      if (this.config.showImageGallery) {
        ...
        this.loading.hide();
        domClass.add(attachmentContainer, "esriCTHidden");
        this._getFeatureInfoLastNode();
      }
    },

    /**
    * reset components of swiper
    * @memberOf widgets/NearMe/item-list
    **/
    resetGallery: function () {
      if (this._currentPanelName === "infoPanel") {
        if (!this._tabContainer || (this._tabContainer &&
          this._tabContainer.selected === this.nls.informationTabTitle)) {
          if (this._gallerySwiper) {
            array.forEach(this._gallerySwiper, function (gallerySwiper) {
              if (gallerySwiper) {
    ...
    **/
    _createGallery: function (layer, attachmentContainer, infos, popupRenderer) {
      ...
      //Clear existing media intervals
      this._clearMediaRefreshTimers();
      ...
      if (infos && infos.length > 0 ||
        ...
        if (layer.infoTemplate && layer.infoTemplate.info &&
          layer.infoTemplate.info.mediaInfos) {
        ...
        if (infos && infos.length > 0) {
          ...
          for (i = 0; i < infos.length; i++) {
            if (addedURL.indexOf(infos[i].url) < 0) {
              addedURL.push(infos[i].url);
              ...
              if (infos[i].contentType &&
                ...
                this._attachEventOnImage(slideContent, null);
                hasSlides = true;
                ...
                this._attachEventOnImage(nonImageAttachmentContainer, null);
              }
            }
          }
        }
        ...
        this._charts = [];
        this._checkMediaInfos(swiperWrapper, popupRenderer, addedURL).then(
          lang.hitch(this, function (hasMediaInfos) {
            ...
            if (hasSlides || hasMediaInfos) {
              ...
              gallerySwiper = new Swiper(swiperContainer, {
                ...
                spaceBetween: 30,
                loop: true,
                a11y: true //This parameter enables accessibility feature for swiper gallery
              });
              ...
              //If duplicate slides have refresh interval, attach event to refresh image
              this.own(on(gallerySwiper, "imagesReady", lang.hitch(this, function () {
                array.forEach(query(".swiper-slide-duplicate", swiperWrapper), lang.hitch(this,
                  function (duplicateNode) {
                    var refreshInterval, imgNode;
                    imgNode = query(".esriCTAttachmentImg", duplicateNode);
                    if (imgNode && imgNode.length > 0) {
                      imgNode = imgNode[0];
                      refreshInterval = domAttr.get(imgNode, 'refreshInterval');
                      this._attachEventOnImage(imgNode, refreshInterval);
                    }
                  }));
              })));
            ...
            this._gallerySwiper.push(gallerySwiper);
            this._getFeatureInfoLastNode();
          }));
      } else {
        this._gallerySwiper.push(null);
        this._getFeatureInfoLastNode();
      }
    },
    ...
    */
    _getFeatureInfoLastNode: function () {
      var lastFocusNode, allNodesInFeatureInfo, paginationBullets;
      if (domStyle.get(this.proximityButton.parentElement, "display") === "block") {
        lastFocusNode = this.proximityButton;
      }
      if (!lastFocusNode) {
        for (var i = this._allContentPanes.length - 1; i >= 0; i--) {
          paginationBullets = query(".swiper-pagination-bullet",
            this._allContentPanes[i].domNode);
          if (paginationBullets && paginationBullets.length > 0) {
            lastFocusNode = paginationBullets.reverse()[0];
          } else {
            allNodesInFeatureInfo =
              jimuUtils.getFocusNodesInDom(this._allContentPanes[i].domNode);
      ...
      //Set info popup as the last focus node
      if (!lastFocusNode && this._featureInfoPanel && this._featureInfoPanel.domNode) {
        lastFocusNode = this._featureInfoPanel.domNode;
      }
      if (!lastFocusNode) {
        lastFocusNode = this._getInformationTabAsLastFocusNode();
        if (lastFocusNode) {
          return;
        }
      }
      ...
      this.emit("setLastNodeInFeatureInfo", lastFocusNode);
    },
    ...
    * @memberOf widgets/NearMe/item-list
    */
    _checkMediaInfos: function (swiperWrapper, popupRenderer, addedURL) {
      ...
      if (mediaInfos && popupRenderer._mediaInfos && popupRenderer._mediaInfos.length > 0) {
        setTimeout(lang.hitch(this, function () {
          ...
          for (var i = 0; i < mediaInfos.length; i++) {
            var mediaInfo = mediaInfos[i], refreshInterval = 0;
            ...
            if (mediaInfo.type === "image" ||
              ...
              if (addedURL.indexOf(mediaInfo.value.sourceURL) < 0) {
                addedURL.push(mediaInfo.value.sourceURL);
                ...
                if (mediaInfo.type === "image") {
                  //set refresh interval if it is set in webMap popupInfo
                  if (mediaInfo.hasOwnProperty('refreshInterval')) {
                    refreshInterval = mediaInfo.refreshInterval;
                  }
                  slideContent = domConstruct.create("img", {
                  ...
                  //set refresh interval as an attribute
                  if (refreshInterval) {
                    domAttr.set("refreshInterval", refreshInterval);
                  }
                  this._attachEventOnImage(slideContent, refreshInterval);
                } else {
                  //load media chart
    ...
    * @memberOf widgets/NearMe/item-list
    **/
    _attachEventOnImage: function (imageDiv, refreshIntervalTime) {
      var refreshTimer;
      // Show image in new tab on click of the image thumbnail
      this.own(on(imageDiv, "click", lang.hitch(this, function () {
        this._displayImageAttachments(imageDiv, false);
      })));
      // Show image in new tab on click of the image thumbnail
      this.own(on(imageDiv, "keydown", lang.hitch(this, function (evt) {
        this._displayImageAttachments(imageDiv, true, evt);
      })));
      if (refreshIntervalTime) {
        //referesh interval time is in minutes convert it to miliseconds
        refreshIntervalTime = 60000 * refreshIntervalTime;
        refreshTimer = setInterval(lang.hitch(this, function () {
          imageDiv.setAttribute("src", imageDiv.alt + "?" + Math.random());
        }), refreshIntervalTime);
        //store timers to clear on going back from info panel
        this._mediaRefreshTimers.push(refreshTimer);
      }
    },

    /**
     * Clears media refresh interval timers
     */
    _clearMediaRefreshTimers: function () {
      if (this._mediaRefreshTimers) {
        array.forEach(this._mediaRefreshTimers, function (timer) {
          clearInterval(timer);
        });
      }
      this._mediaRefreshTimers = [];
    },
    ...
    * @memberOf widgets/NearMe/item-list
    **/
    _displayImageAttachments: function (target, isKeyPressed, evt) {
      if (isKeyPressed && (evt.keyCode !== keys.ENTER && evt.keyCode !== keys.SPACE)) {
        return;
      }
      window.open(domAttr.get(target, "src"));
    },
    ...
    **/
    _initializeDirectionWidget: function () {
      ...
      if (!this._directionsWidget) {
        if (registry.byId("directionDijit" + this.id)) {
          registry.byId("directionDijit" + this.id).destroy();
        }
        //configure direction parameters
        directionParams = {
          id: "directionDijit" + this.id,
          map: this.map,
        ...
        //Set the printing service so that map can be printed - #602
        if (this.config.helperServices && this.config.helperServices.printTask && this.config.helperServices.printTask.url) {
          directionParams.printTaskUrl = this.config.helperServices.printTask.url;
        }
        ...
        this.own(this._directionsWidget.on("directions-finish", lang.hitch(this, function () {
          ...
          this.loading.hide();
          //TODO: check if getFocusNodesInDom retruns the node, before using [0]
          //(check this at all places)
          if (domStyle.get(this.proximityButton.parentElement, "display") === "block") {
            jimuUtils.initLastFocusNode(this.domNodeObj, this.proximityButton);
          } else {
            this.emit("setLastNodeInFeatureInfo",
    ...
    **/
    _routeSelectedLocations: function () {
      ...
      this._clearDirections();
      if (this._selectedPoint && this._selectedFeature && this._selectedFeature.geometry) {
    ...
    */
    ...
    _getHighLightSymbolColor: function () {
      //use defalut cyan
      var highlightColor = new Color([0, 255, 255, 1]);
      //use configured highlight color if configured
      if (this.config.highlightColor) {
        highlightColor = new Color(this.config.highlightColor);
        if (!this.config.hasOwnProperty('showSelectionColor')) {
          this.config.showSelectionColor = true;
        }
        //update alpha value if it is configured
        if (this.config.showSelectionColor) {
          highlightColor.a = 1;
        } else {
          highlightColor.a = 0;
        }
    ...
    */
    _getPointSymbol: function (graphic, layer) {
      var symbol, isSymbolFound, graphics, point, graphicInfoValue,
        layerInfoValue, i, highlightColor;
      isSymbolFound = false;
      highlightColor = this._getHighLightSymbolColor();
    ...
    */
    _getPolyLineSymbol: function (graphic, layer) {
      ...
      //use configured highlight color else use defalut cyan
      highlightColor = this._getHighLightSymbolColor();
      //check if layer is valid and have valid renderer object then only check for other  symbol properties
      if (layer && layer.renderer) {
      ...
      symbol = new SimpleLineSymbol(SimpleLineSymbol.STYLE_SOLID, highlightColor, symbolWidth);
    ...
    */
    _getPolygonSymbol: function (graphic) {
      var symbol, graphics, polygon, highlightColor;
      //use configured highlight color else use defalut cyan
      highlightColor = this._getHighLightSymbolColor();
      symbol = new SimpleFillSymbol(SimpleFillSymbol.STYLE_SOLID, new SimpleLineSymbol(
        SimpleLineSymbol.STYLE_SOLID, highlightColor, 4),
        new Color([0, 0, 0, 0]));
    ...
    * @memberOf widgets/NearMe/item-list
    **/
    showLayersOnBackToLayerList: function () {
      var i, showLayer;
      for (i = 0; i < this.config.searchLayers.length; i++) {
        if (this.config.selectedSearchLayerOnly) {
          if (this._searchedFeatures[this.config.searchLayers[i].id] &&
            this._searchedFeatures[this.config.searchLayers[i].id].length > 0) {
            showLayer = true;
          } else {
            showLayer = false;
          }
    ...
    * @memberOf widgets/NearMe/item-list
    **/
    _getConfiguredSymbologyForLayer: function (selectedLayerId) {
      var configuredSymbols = [];
      //for backward compatibility attribute symbology will be the object
      //which will conatin array of symbology for each layerid
      //For new config's from 7.2 onwards to maintain the order of symbol
      //attributeSymbology will be be an array with each object containin layer and other deatils
      //so create array of only those symbols configured for the selected layer and return it
      if (this.config.attributeSymbology &&
        this.config.attributeSymbology.hasOwnProperty(selectedLayerId)) {
        return this.config.attributeSymbology[selectedLayerId];
      } else if (this.config.attributeSymbology && this.config.attributeSymbology.length &&
        this.config.attributeSymbology.length > 0) {
        array.forEach(this.config.attributeSymbology, lang.hitch(this, function (currentObj) {
          if (currentObj.hasOwnProperty('layerId') &&
            currentObj.layerId === selectedLayerId) {
            configuredSymbols.push(currentObj);
          }
        }));
      }
    ...
    * @memberOf widgets/NearMe/item-list
    **/
    _checkAttributeSymbology: function (selectedFeature, attributeSymbology) {
      ...
      array.forEach(attributeSymbology,
    ...
    **/
    ...
    _attachEventToTab: function () {
      if (this._tabContainer !== '' && this._tabContainer !== null && this._tabContainer !== undefined) {
        if (this._tabContainer.controlNodes && this._tabContainer.controlNodes.length > 1) {
          var directionTab = this._tabContainer.controlNodes[1];
          this.own(on(directionTab, 'keydown', lang.hitch(this, function (evt) {
            if ((evt.keyCode === keys.TAB) && (!evt.shiftKey)) {
              if (this._tabContainer.viewStack) {
                var selectedTabTitle = this._tabContainer.viewStack.getSelectedLabel();
                if (selectedTabTitle === this.nls.informationTabTitle) {
                  if (!this.config.enableProximitySearch) {
                    Event.stop(evt);
                    this.emit("setFocusOnFirstFocusableNode");
                  } else {
                    Event.stop(evt);
                    focusUtil.focus(this.proximityButton);
                  }
                }
              }
            }
    ...
    /**
     * This function is used to set the information tab as a last focus node
     */
    _getInformationTabAsLastFocusNode: function () {
      // After search complete, list of layer gets displayed. On click of layer item a new screen appears
      // which displays information & direction tab. Now, if information tab is active, than information tab
      // should be set as a last focus node. So that, after layer name, focus sets on information tab.
      if (this._tabContainer !== '' && this._tabContainer !== null && this._tabContainer !== undefined) {
        if (this._tabContainer.viewStack) {
          var selectedTabTitle = this._tabContainer.viewStack.getSelectedLabel();
          if (selectedTabTitle === this.nls.informationTabTitle) {
            if (this._tabContainer.controlNodes && this._tabContainer.controlNodes.length > 0) {
              var lastFocusNode = this._tabContainer.controlNodes[0];
              if (!this.config.enableProximitySearch) {
                jimuUtils.initLastFocusNode(this.domNodeObj, lastFocusNode);
                return true;
              }
            }
          }
