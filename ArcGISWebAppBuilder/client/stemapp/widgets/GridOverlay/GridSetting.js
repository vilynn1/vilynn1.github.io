...
define([
  ...
  function (
    ...
    var setColorText = function (domNode, color) {
      ...
      domNode.innerHTML = jimuUtils.sanitizeHTML("<div class='" + domNodeClass + "' style='" +
        style + "'>" + text + "</div>");
    };
    ...
    return declare([dijitWidgetBase, dijitTemplatedMixin, dijitWidgetsInTemplate], {
      ...
      postCreate: function () {
        ...
        this.minIntervalSpacing.onChange = lang.hitch(this, function (val) {
          var width = (!window.isRTL) ? val + 'px' : 'px' + val;
          this.minIntervalSpacingVal.innerHTML = jimuUtils.sanitizeHTML(width);
        ...
        this.lineOpacity.onChange = lang.hitch(this, function (val) {
          this.lineOpacityVal.innerHTML = (!window.isRTL) ? jimuUtils.sanitizeHTML(Math.round(val * 100) + '%') :
            jimuUtils.sanitizeHTML('%' + Math.round(val * 100));
          grid.setLineOpacity(val);
        });
        this.labelOpacity.onChange = lang.hitch(this, function (val) {
          this.labelOpacityVal.innerHTML = (!window.isRTL) ? jimuUtils.sanitizeHTML(Math.round(val * 100) + '%') :
            jimuUtils.sanitizeHTML('%' + Math.round(val * 100));
        ...
        // Code for Accessibility: keydown event for verticalLabels
        //and horizontalLabels button
        this.verticalLabels.onkeydown = function (evt) {
        ...
        this.horizontalLabels.onkeydown = function (evt) {
          if (evt.keyCode === keys.SPACE || evt.keyCode === keys.ENTER) {
            evt.stopPropagation();
            evt.preventDefault();
            self.verticalLabelsClicked = false;
        ...
        this._formatNumberTolocale(100000, this.nls.gsid, this.gsidNode);
        this._formatNumberTolocale(10000, this.nls.grid10k, this.grid10kNode);
        this._formatNumberTolocale(1000, this.nls.grid1k, this.grid1kNode);
      },
      ...
      setConfig: function (origConfig) {
        ...
        this.minIntervalSpacingVal.innerHTML = (!window.isRTL) ?
          ...
          jimuUtils.sanitizeHTML('px' + this.config.minIntervalSpacing);
        // this.minIntervalDisplay.style.width = this.config.minIntervalSpacing + 'px';
        this.lineOpacity.set('value', this.config.lineOpacity);
        this.lineOpacityVal.innerHTML = (!window.isRTL) ?
          jimuUtils.sanitizeHTML(Math.round(this.config.lineOpacity * 100) + '%') :
          jimuUtils.sanitizeHTML('%' + Math.round(this.config.lineOpacity * 100));
        this.labelOpacity.set('value', this.config.labelOpacity);
        this.labelOpacityVal.innerHTML = (!window.isRTL) ?
          jimuUtils.sanitizeHTML(Math.round(this.config.labelOpacity * 100) + '%') :
          jimuUtils.sanitizeHTML('%' + Math.round(this.config.labelOpacity * 100));
      ...
      _formatNumberTolocale: function (number, nls, node) {
        var formattedNumberStr = string.substitute(nls, {
          "number": jimuUtils.localizeNumber(number)
        });
        domAttr.set(node, "innerHTML", formattedNumberStr);
      }
    });
  });