...
define([
  ...
  var proximityInfo = declare('ProximityInfo', null, {
      ...
      array.forEach(this.tab.tabLayers, lang.hitch(this, function (tab) {
        ...
        if (tab.url) {
          ...
          on(tempFL, "load", lang.hitch(this, function () {
            this.tab.tabLayers = [tempFL];
            if (isSnapShot) {
              this.processIncident(incidents, distance, graphicsLayer, snapShot, createSnapShot).then(lang.hitch(this,
                function (results) {
                def.resolve(results);
              ...
              this.processIncident(incidents, distance, graphicsLayer, snapShot, createSnapShot);
            }
          }));
        } else {
          if (isSnapShot) {
    ...
    processIncident: function (incidents, buffers, graphicsLayer, snapShot, createSnapShot) {
      ...
      var configuredPopUpFields = analysisUtils.getPopupConfiguredFields(layer);
          ...
          if (featureSet && featureSet.features) {
            ...
            for (var g = 0; g < graphics.length; g++) {
              ...
              array.forEach(fields, lang.hitch(this, function (field) {
                newAttr[field] = gra.attributes[field];
              }));
              ...
              if (((this.config.hasOwnProperty("exportFieldsOptionForCSV") &&
                ...
                (this.config.csvAllFields === true || this.config.csvAllFields === "true"))) {
                //do nothing.  All fields in graphic will export.
                gra.attributes.DISTANCE = dist;
              } else if ((this.config.hasOwnProperty("exportFieldsOptionForCSV") &&
                this.config.exportFieldsOptionForCSV === "popUpFields")) {
                //if create snapshot button is clicked: All fields are required
                if (createSnapShot) {
                  //In case of create snapshot: do nothing
                  gra.attributes = newAttr;
                } else {
                  //In case of csv download: filter fields which are configured in popup.
                  newAttr = {
                    DISTANCE: dist
                  };
                  array.forEach(configuredPopUpFields, lang.hitch(this, function (field) {
                    newAttr[field] = gra.attributes[field];
                  }));
                  //popUp configured fields in graphic will export
                  gra.attributes = newAttr;
                }
              }
              else {
    ...
    _processResults: function (results, report) {
      ...
      if (report) {
        ...
        if (hasResults) {
          ...
          //Listen for ENTER and SPACE button click
          on(div_exp, "keydown", lang.hitch(this, function (evt) {
            //Skip the tab + shift button click
        ...
        for (var i = 0; i < results.length; i++) {
                ...
                if (prop !== "DISTANCE" && c < 3) {
                  if (field.expression === prop) {
                    ...
                    var fVal = analysisUtils.getFieldValue(prop, attr[prop], this.specialFields,
                      this.dateFields, 'longMonthDayYear', this.typeIdField, this.types, lyr, attr, field);
                    var value;
                    ...
                    var label = typeof(field.label) !== 'undefined' && field.label !== '' ? field.label : undefined;
                    ...
                    if (_fields && typeof(label) === 'undefined') {
                    ...
                    info += field.validLabel ?
                      ((typeof (field.label) !== 'undefined' && field.label !== '' ? label + ' ' : '') +
                        value + '<br/>') : value + '<br/>';
                    c += 1;
          ...
          if (!report) {
            ...
            var div2 = domConstruct.create("div", {
              ...
              "tabindex": "0",
              "role": "button",
              "aria-label": this.parent.nls.zoomToFeature + " " + num,
              "isLastElement": results.length - 1 === i
            }, div1);
            domClass.add(div2, "SATcolRecNum");
            if (results.length - 1 === i) {
              utils.initLastFocusNode(this.parentNode, div2);
            }
            domStyle.set(div2, "backgroundColor", this.parent.config.activeColor);
            on(div2, "click", lang.hitch(this, this._zoomToLocation, loc, null));
            on(div2, "keydown", lang.hitch(this, this._zoomToLocation, loc, true));
            ...
            if (this.parent.config.enableRouting) {
              ...
              on(div4, "click", lang.hitch(this, this._routeToIncident, loc, null));
              on(div4, "keydown", lang.hitch(this, this._routeToIncident, loc, true));
              utils.initLastFocusNode(this.parentNode, div4);
            }
            ...
            fnt.family = "Arial";
            fnt.size = "12px";
            var symTextColor = this.parent.lightTheme ? "#111111" : this.parent.config.fontColor;
            var symText = new TextSymbol(num, fnt, new esriColor(symTextColor));
    ...
    _zoomToLocation: function (loc, isKeyPress, evt) {
      if (evt.shiftKey && evt.keyCode === keys.TAB) {
        return;
      }
    ...
    _routeToIncident: function (loc, isKeyPress, evt) {
      if (isKeyPress && !(evt.keyCode === keys.ENTER || evt.keyCode === keys.SPACE)) {
        return;
      }
