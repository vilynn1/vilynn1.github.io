  ...
  var closestInfo = declare('ClosestInfo', null, {
    ...
    queryTabCount: function (incidents, buffers, updateNode, displayCount) { // jshint ignore:line
      ...
      for (var i = 0; i < incidents.length; i++) {
        var geom = incidents[i].geometry;
        if (geom.spatialReference.wkid === "4326" || geom.spatialReference.isWebMercator()) {
          buffers.push(geometryEngine.geodesicBuffer(geom, distance, unitCode));
        } else {
          buffers.push(geometryEngine.buffer(geom, distance, unitCode));
        }
      }
    ...
    processIncident: function (incidents, distance, graphicsLayer, snapShot) {
      ...
      for (var j = 0; j < incidents.length; j++) {
        ...
        var bufferGeom;
        if (geom.spatialReference.wkid === "4326" || geom.spatialReference.isWebMercator()) {
          bufferGeom = geometryEngine.geodesicBuffer(geom, distance, unitCode);
        } else {
          bufferGeom = geometryEngine.buffer(geom, distance, unitCode);
        }
