...
define([
  ...
  function(
    ...
    FieldPicker, esriLang, FeatureLayerChooserFromMap, BaseWidgetSetting,
    Message, Popup, TabContainer3, GpSource, LayerInfos,
    CheckBox, domClass, domConstruct, keys, registry, ImageChooser, portalUtils, jimuUtils, LayerChooserFromMapWithDropbox
  ) {
    return declare([BaseWidgetSetting, _WidgetsInTemplateMixin, Evented], {
      ...
      _initCheckBoxes: function () {
        this.chk_routing = this._initCheckBox(this.nls.routing_checkBox, this.div_chk_routing);
        this.chk_visibility = this._initCheckBox(this.nls.visibility_checkBox, this.div_chk_visibility);
        this.chk_display = this._initCheckBox(this.nls.display_checkbox, this.div_chk_display);
        this.chk_save = this._initCheckBox(this.nls.save_checkbox, this.div_chk_save);
        this.chk_snapshot = this._initCheckBox(this.nls.snapshot_checkbox, this.div_chk_snapshot);
        this.chk_point_save = this._initCheckBox(this.nls.save_point_checkbox, this.div_chk_point_save);
        this.chk_line_save = this._initCheckBox(this.nls.save_line_checkbox, this.div_chk_line_save);
        this.chk_poly_save = this._initCheckBox(this.nls.save_poly_checkbox, this.div_chk_poly_save);
        this.chk_report = this._initCheckBox(this.nls.report_checkbox, this.div_chk_report);
        this.chk_geodesic = this._initCheckBox(this.nls.drawGeodesic, this.div_chk_geodesic);
      },
      ...
      _initReport: function () {
        this.own(on(this.chk_report, 'change', lang.hitch(this, function (v) {
      ...
      setConfig: function(config) {
        ...
        if (this.config.distanceUnits) {
          this.selectUnits.setValue(this.config.distanceUnits);
        }

        if (this.config.maxDistance) {
          this.txt_maximumDistance.setValue(this.config.maxDistance);
        }

        if (this.config.enableRouting) {
          this.chk_routing.setValue(true);
        }

        if (this.config.drawGeodesic) {
          this.chk_geodesic.setValue(true);
        }
        ...
        for (var i = 0; i < this.config.tabs.length; i++) {
          ...
          var missingLayer = this._populateTabTableRow(aTab);
          if (missingLayer) {
            missingLayers.push(missingLayer);
          }
        }

        this._handleMissingLayers(missingLayers);
        ...
        if (this.config.disableVisibilityManagement) {
          this.chk_visibility.setValue(true);
        }
        ...
        if (!hasEditableLayers) {
          this.chk_save.setStatus(false);
          this._updateDisplay(this.editOptions, false);
        } else {
          this.own(on(this.chk_save, 'change', lang.hitch(this, function (v) {
            this._updateDisplay(this.editOptions, v);
          })));
          this.chk_save.setValue(this.config.saveEnabled);
        ...
        if (this.config.hasOwnProperty("exportFieldsOptionForCSV")) {
          if (this.config.exportFieldsOptionForCSV === "analysis") {
            this.analysisFieldsRadioButton.set("checked", true);
          } else if (this.config.exportFieldsOptionForCSV === "popUpFields") {
            this.PopUpFieldsRadioButton.set("checked", true);
          } else {
            this.allFieldsRadioButton.set("checked", true);
          }
        } else if (this.config.hasOwnProperty("csvAllFields")) {
          if (this.config.csvAllFields) {
            this.allFieldsRadioButton.set("checked", true);
          } else {
            this.analysisFieldsRadioButton.set("checked", true);
          }
        ...
        this.chk_display.setValue(this.config.summaryDisplayEnabled);

        //set selection color if configured
        //else get the selected theme color and show in colorPicker
        if (this.config.selectionColor) {
          this.selectionColorPicker.setColor(new Color(this.config.selectionColor));
        } else {
          jimuUtils.getStyleColorInTheme().then(lang.hitch(this, function (styleColor) {
            if (styleColor) {
              this.selectionColorPicker.setColor(new Color(styleColor));
            }
           }));
        }

        //Snapshot settings
        this.chk_snapshot.setValue(this.config.snapshotEnabled || false);

        //Report Settings
        this.chk_report.setValue(this.config.reportEnabled || false);
      ...
      _handleMissingLayers: function (missingLayers) {
      ...
      _validateSave: function () {
        var valid = false;
        if (this.chk_save.getValue()) {
          if (this.chk_point_save.getValue() || this.chk_line_save.getValue() || this.chk_poly_save.getValue()) {
            valid = true;
          }
        } else {
          valid = true;
      ...
      getConfig: function() {
        ...
        this.config.disableVisibilityManagement = this.chk_visibility.getValue();
        this.config.enableRouting = this.chk_routing.getValue();
        this.config.saveEnabled = this.chk_save.getValue();
        this.config.savePoints = this.chk_point_save.getValue();
        this.config.drawGeodesic = this.chk_geodesic.getValue();
        if (this.config.savePoints) {
          this.config.pointEditLayer = this.selectPointSaveLayer.value;
        }
        this.config.saveLines = this.chk_line_save.getValue();
        if (this.config.saveLines) {
          this.config.lineEditLayer = this.selectLineSaveLayer.value;
        }
        this.config.savePolys = this.chk_poly_save.getValue();
        ...
        this.config.selectionColor = this.selectionColorPicker.getColor().toHex();

        if (this.allFieldsRadioButton.checked) {
          this.config.exportFieldsOptionForCSV = "allFields";
        }
        if (this.PopUpFieldsRadioButton.checked) {
          this.config.exportFieldsOptionForCSV = "popUpFields";
        }
        if (this.analysisFieldsRadioButton.checked) {
          this.config.exportFieldsOptionForCSV = "analysis";
        }

        this.config.summaryDisplayEnabled = this.chk_display.getValue();
        this.config.snapshotEnabled = this.chk_snapshot.getValue();
        this.config.reportEnabled = this.chk_report.getValue();
      ...
      _setLayers: function () {
        ...
        var polySaveOptions = [];
        this.skipLayers = [];
        array.forEach(this.opLayers.getLayerInfoArray(), lang.hitch(this, function (OpLyr) {
          if (OpLyr.newSubLayers.length > 0) {
            ...
            this._checkSkipLayer(OpLyr).then(lang.hitch(this, function (isSkipLayer) {
              if (!isSkipLayer) {
                options.push({
                  label: OpLyr.title,
                  value: OpLyr.id
        ...
        if (this.skipLayers.length > 0) {
          ...
          array.forEach(this.skipLayers, function (l) {
            if (l.type !== null && l.type !== undefined) {
              //TODO update nls for 5.4
              _type = l.type === 'collection' ? 'Feature Collection' : l.type;
            } else {
              _type = "";
            }
      ...
      checkFields: function () {
        ...
        for (var i = 0; i < this.layer_options.length; i++) {
          ...
          fp = new FieldPicker({
            nls: this.nls,
            callerLayer: l.value,
            callerOpLayers: this.opLayers.getLayerInfoArray(),
            map: this.map,
            test: true,
            callerTab: {
              type: 'closest'
      ...
      _checkSkipLayer: function (opLyr) {
        ...
        var supportedLayerTypes = ["ArcGISFeatureLayer", "ArcGISMapServiceLayer", "Feature Layer", "FeatureLayer"];
        var skipLayer = false;
        opLyr.getLayerType().then(lang.hitch(this, function (layerType) {
          if (opLyr.layerObject) {
            ...
            if (layerType && supportedLayerTypes.indexOf(layerType) === -1) {
              skipLayer = true;
            }
      ...
      _populateTabTableRow: function (tabInfo) {
        ...
        if (layerInfo) {
          ...
          return tabInfo.layerTitle || tabInfo.layers;
        }
      },
      ...
      validatePattern: function(v){
        return !isNaN(jimuUtils.parseNumber(jimuUtils.localizeNumber(v)));
      },

      checkMax: function (d, v) {
        v = v.toString();
        var _v = jimuUtils.parseNumber(jimuUtils.localizeNumber(v));
        var _minV = jimuUtils.parseNumber(jimuUtils.localizeNumber(this.buffer_min.value));
      ...
      checkBetween: function (d, v) {
        ...
        v = v.toString();
        var _v = jimuUtils.parseNumber(jimuUtils.localizeNumber(v));
        var _maxV = jimuUtils.parseNumber(jimuUtils.localizeNumber(this.buffer_max.value));
