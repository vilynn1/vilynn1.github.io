...
define([
  ...
  function (declare, _WidgetsInTemplateMixin, Button,
    BaseWidget, Message, utils, accessibleUtils, LayerInfos, portalUtils, Report, pageUtils,
    Color, html, dom, on, domStyle, domClass, domConstruct, domAttr, domGeom, lang, array, xhr,
    query,
    JSON,
    topic,
    aspect,
    ...
    return declare([BaseWidget, _WidgetsInTemplateMixin], {
      ...
      postCreate: function() {
        ...
        domAttr.set(this.closeButton, "aria-label", this.nls.close);
        //Keydown event for widget panel
        ...
        //Keydown event for tab contents
        this.own(on(this.panelContainer, "keydown", lang.hitch(this, function (evt) {
          if (evt.keyCode === keys.ESCAPE) {
            evt.stopPropagation();
            query(".SATTab.active", this.domNode)[0].focus();
          }
        })));
        //Keydown event for close button
        this.own(on(this.closeButton, "keydown", lang.hitch(this, function (evt) {
        ...
        if (utils.has('chrome')) {
          domStyle.set(this.tabsNode, "position", "absolute");
        }
        //In dart theme the closest address panel is shown on load
        if (this.div_reverse_geocoding) {
          html.setStyle(this.div_reverse_geocoding, 'display', 'none');
        }
      },
      ...
      startup: function() {
        ...
        this.SLIDER_MAX_VALUE = utils.parseNumber(utils.localizeNumber(this.config.bufferRange.maximum.toString()));
        this.SLIDER_MAX_VALUE_DISPLAY = utils.localizeNumber(this.config.bufferRange.maximum.toString());
        this.SLIDER_MIN_VALUE = utils.parseNumber(utils.localizeNumber(this.config.bufferRange.minimum.toString()));
        this.SLIDER_MIN_VALUE_DISPLAY = utils.localizeNumber(this.config.bufferRange.minimum.toString());
        if(typeof (this.config.bufferRange._default) === 'undefined'){
          this.config.bufferRange._default = this.config.bufferRange.minimum;
        }
        this.SLIDER_DEFAULT_VALUE = utils.parseNumber(utils.localizeNumber(this.config.bufferRange._default.toString()));
      ...
      onOpen: function() {
        ...
        //Set focus to first focus node only when widget is opened after clicking on 
        //the widget icon
        if (!this.isInitialLoad && utils.isAutoFocusFirstNodeWidget(this)) {
      ...
      _setEventLocation: function (results) {
        ...
        domClass.remove(this.panelContainer, "loading");
      },
      ...
      _initLayers: function () {
        ...
        //Shifted this code to make sure the buffer layer is always displayed on top
      ...
      _setEditLayerPopup: function (f) {
        ...
        if(utils.isMobileUa()) {
          var inspectorFooterButtonArray = query(".esriMobileInfoViewItem .esriAttributeInspector .atiDeleteButton");
          if(inspectorFooterButtonArray && inspectorFooterButtonArray.length > 0) {
            domClass.add(this.attInspector.deleteBtn.domNode, "esriCTAIDeleteButtonWidth");
          }
        }
      ...
      _goToNextTab: function (evt, currentTabNum) {
        var newTabNum, tabElement;
        //Check for left arrow button key press or shift + tab key press
        if (evt.keyCode === keys.LEFT_ARROW || evt.shiftKey && evt.keyCode === keys.TAB) {
          if (currentTabNum === 0) {
            newTabNum = this.tabNodes.length - 1;
          } else {
            newTabNum = currentTabNum - 1;
          }
        } else if (evt.keyCode === keys.RIGHT_ARROW || (evt.keyCode === keys.TAB)) {
          //Check for right arrow button key press or tab key press
          if (currentTabNum === this.tabNodes.length - 1) {
            newTabNum = 0;
          } else {
            newTabNum = currentTabNum + 1;
          }
        } else {
          //Bypass the code if any other key is pressed
          return;
        }

        tabElement = query("div[tabNum=" + newTabNum + "]", this.domNode)[0];
        //Check if tab is disabled before setting focus
        if (domClass.contains(tabElement, "noFeaturesActive")) {
          tabElement = this._goToNextTab(evt, newTabNum);
        }
        return tabElement;
      },
      ...
      _createUI: function () {
        ...
        this.spinnerValue.isRenderIdForAttrs = true;
        ...
        var msg = esriLang.substitute({
          min: this.SLIDER_MIN_VALUE_DISPLAY,
          max: this.SLIDER_MAX_VALUE_DISPLAY
        },
        ...
        for (var i = 0; i < this.config.tabs.length; i++) {
          ...
          var tab = domConstruct.create("div", {
            ...
            "tabNum": i,
            "role":"tab",
            "aria-label": utils.stripHTML(label ? label : ''),
            "aria-selected": "false",
          ...
          on(tab, "keydown", lang.hitch(this, function (evt) {
            var nextTabElement, currentTabNum;
            currentTabNum = parseInt(domAttr.get(evt.currentTarget, "tabNum"), 10);
            if (evt.keyCode === keys.TAB) {
              evt.preventDefault();
              nextTabElement = this._goToNextTab(evt, currentTabNum);
              nextTabElement.focus();
            } else if (evt.keyCode === keys.LEFT_ARROW || evt.keyCode === keys.RIGHT_ARROW) {
              nextTabElement = this._goToNextTab(evt, currentTabNum);
              nextTabElement.focus();
            } else if (evt.keyCode === keys.ENTER || evt.keyCode === keys.SPACE) {
              this._clickTab(parseInt(domAttr.get(evt.currentTarget, "tabNum"), 10));
              if (domAttr.get(evt.currentTarget, "tabNum") === "0") {
                setTimeout(lang.hitch(this, function () {
          ...
          if (i > 0) {
            ...
            on(this.panelNodes[i], "keydown", lang.hitch(this, function (evt) {
              if (evt.keyCode === keys.ESCAPE) {
                array.forEach(this.tabNodes, lang.hitch(this, function (node) {
        ...
        if (wTabs > domGeom.position(this.footerNode).w) {
          ...
          if (window.isRTL) {
            domStyle.set(fc, 'left', "58" + "px");
            domStyle.set(this.panelLeft, 'display', 'block');
            domStyle.set(this.panelRight, 'display', 'none');
          } else {
        ...
        this.own(on(this.clearIncident, "keydown", lang.hitch(this, function (evt) {
          if (evt.keyCode === keys.ENTER || evt.keyCode === keys.SPACE) {
            this._clear();
            this.SA_btn0.focus();
      ...
      _loadUI: function () {
        ...
        this.own(on(this.spinnerValue, "change", lang.hitch(this, function () {
          //Set the number spinner update flag to true
          //This will help in determining wether to set the focus to the number
          this._updateSpinnerValue(false);
        })));

        this.own(on(this.spinnerValue, "blur", lang.hitch(this, function () {
          this._updateSpinnerValue(true);
        })));

        this.own(on(this.spinnerValue, "keyup", lang.hitch(this, function (evt) {
        ...
        defList.then(lang.hitch(this, function (defResults) {
          ...
          for (var r = 0; r < defResults.length; r++) {
            ...
            if (featureSet.success) {
              ...
              errorMessge = defResults[r][1][0].error.message;
              new Message({
                title: this.nls.saveIncident,
                message: this.nls.saveIncidentErrorMessage + errorMessge
              });
          }
          }
          this._smartEdit();
      ...
      _updateSpinnerValue: function (set) {
        if (this.spinnerValue.validate()) {
          ...
          if (typeof (num) === 'string') {
            num = utils.parseNumber(num.toString());
          }
      ...
      _clickTab: function (num) {
        if (this._validateFeatureCount(num)) {
          domAttr.set(this.tabNodes[num], "aria-selected", "true");
          array.forEach(this.tabNodes, lang.hitch(this, function (node, index) {
            if (num !== index) {
              domAttr.set(node, "aria-selected", "false");
            }
          }));
          ...
          this._clickIncidentsButton(-1);
          setTimeout(lang.hitch(this, function () {
            this._getLastNodeForEachTab(num);
          }), 100);
        }
      },

      _getLastNodeForEachTab: function (num) {
        var lastNode = this._getLastFocusNodeInDom(num);
        if (lastNode) {
          if (num === 0) {
            if (!domClass.contains(this.clearIncident, "display-off")) {
              utils.initFirstFocusNode(this.domNode, this.clearIncident);
            }
            else {
              utils.initFirstFocusNode(this.domNode, this.SA_btn0);
            }
          }
          utils.initLastFocusNode(this.domNode, lastNode.domNode ? lastNode.domNode : lastNode);
        }
      },

      _getLastFocusNodeInDom: function (num) {
        if (num === 0) {
          return this._getLastFocusNodeInIncidentTab();
        }
      },

      _getLastFocusNodeInIncidentTab: function () {
        var lastNode;
        if (!domClass.contains(this.saveOptions, "display-off")) {
          if (this.config.snapshotEnabled && this.createSnapshotButton &&
            !domClass.contains(this.createSnapshotButton, "btnDisabled")) {
            lastNode = this.createSnapshotButton;
          }

          if (!lastNode && this.downloadAllButon &&
            !domClass.contains(this.downloadAllButon, "btnDisabled")) {
            lastNode = this.downloadAllButon;
          }

          if (!lastNode && this.config.saveEnabled && this.saveButton &&
            !domClass.contains(this.saveButton, "btnDisabled")) {
            lastNode = this.saveButton;
          }

          if (!lastNode && this.config.reportEnabled && this.createReportButton &&
            !domClass.contains(this.createReportButton, "btnDisabled")) {
            lastNode = this.createReportButton;
          }
        }

        if (!lastNode && this.spinnerValue) {
          lastNode = this.spinnerValue;
        }
        ...
        switch (tab.type) {
          ...
          case "summary":
            ...
            if (tab.tabLayers) {
              if (tab.tabLayers.length > 1) {
                if (this.lyrSummary) {
                  ...
                  if (tab.tabLayers[0] && tab.tabLayers[0].hasOwnProperty("opacity")) {
                    this.lyrSummary.setOpacity(tab.tabLayers[0].opacity);
                  }
            ...
            if (this.incidents.length > 0 && tab.updateFlag === true) {
              ...
              var summaryDownloadBtn = query(".summaryDownLoadCSVButton", this.domNode);
              if (summaryDownloadBtn && summaryDownloadBtn.length > 0) {
                summaryDownloadBtn[0].focus();
                utils.initFirstFocusNode(this.domNode, summaryDownloadBtn[0]);
                utils.initLastFocusNode(this.domNode, summaryDownloadBtn[0]);
              }
            }
            break;
          case "groupedSummary":
            ...
            if (this.incidents.length > 0 && tab.updateFlag === true) {
              ...
              var groupSummaryDownloadBtn = query(".grpSummaryDownLoadCSVButton", this.domNode);
              if (groupSummaryDownloadBtn && groupSummaryDownloadBtn.length > 0) {
                groupSummaryDownloadBtn[0].focus();
                utils.initFirstFocusNode(this.domNode, groupSummaryDownloadBtn[0]);
                utils.initLastFocusNode(this.domNode, groupSummaryDownloadBtn[0]);
              }
            }
            break;
          case "closest":
      ...
      _updatePanel: function (editEnabled) {
        ...
        if (this.saveEnabled) {
          ...
          if (editEnabled && domClass.contains(this.saveButton, 'btnDisabled')) {
            ...
            domAttr.set(this.saveButton, "tabindex", "0");
          }
      ...
      _showIncidentAddress: function(evt) {
        if (evt.address.address) {
          ...
          html.setStyle(this.div_reverse_geocoding, 'display', 'block');
        }
      },
        ...
        if (this.buffers.length > 0) {
          ...
          if (this.saveEnabled) {
            if (editEnabled && this.userCanSave) {
              ...
              domAttr.set(this.saveButton, "tabindex", "-1");
            }
          }
          this.zoomToIncidents(skipZoom);
        }
        setTimeout(lang.hitch(this, function () {
          query(".noFeaturesActive", this.domNode).forEach(function (node) {
      ...
      _updateCounts: function (clear) {
        ...
        defList.then(lang.hitch(this, function (defResults) {
          ...
          //Update the last focus node in incident tab once the process is completed
          this._getLastNodeForEachTab(0);
        }));
      },
      ...
      _setupSymbols: function() {
        ...
        if (this.config.selectionColor) {
          symColor = Color.fromString(this.config.selectionColor);
        } else {
          symColor = Color.fromString(this.config.color);
        }
      ...
      _onPanelScroll: function (num) {
        ...
        if (window.isRTL) {
          for (var i = 0; i < this.tabsNode.children.length; i++) {
            ...
            var right = cRect.right;
            right = parseFloat(right.toString().split(".")[0]);
            if (right <= rect.right) {
              n = i;
              ss = right;
              break;
          ...
          var showRight = n >= 1 || ss < up;
          domStyle.set(fc, 'right', showRight ? "34px" : "0px");
          domStyle.set(this.panelRight, 'display', showRight ? "block" : "none");
          domStyle.set(this.panelRight, 'width', showRight ? "34px" : "0px");
          var showLeft = nn <= this.tabsNode.children.length;
          domStyle.set(fc, 'left', showLeft ? "58px" : "24px");
          domStyle.set(this.panelLeft, 'display', showLeft ? "block" : "none");
          domStyle.set(this.panelLeft, 'width', showLeft ? "34px" : "0px");
          if (domClass.contains(this.clearIncident, "display-off")) {
            domStyle.set(this.panelLeft, 'display', "none");
            domStyle.set(this.panelRight, 'display', "none");
          }
        } else {
        for (var i = 0; i < this.tabsNode.children.length; i++) {
      ...
      _navTabs: function (right) {
        var rect = this.footerContentNode.getBoundingClientRect();
        if (window.isRTL) {
          for (var i = this.tabsNode.children.length - 1; i >= 0; i--) {
        ...
        s._performAnalysis(analysisObjects, this.incidents, this.buffers, false, true).then(function (r) {
          ...
          array.forEach(r, lang.hitch(this, function (_r) {
            if (_r) {
              ...
              var _rows = [];
              var _rows_ = [];
              if (!_r.context.tab.advStat.hasOwnProperty("analysisSummaryForReport") ||
              _r.context.tab.advStat.hasOwnProperty("analysisSummaryForReport") &&
                _r.context.tab.advStat.analysisSummaryForReport) {
              ...
              if (!_r.context.tab.advStat.hasOwnProperty("PopUpSummaryForReport") ||
              _r.context.tab.advStat.hasOwnProperty("PopUpSummaryForReport") &&
                _r.context.tab.advStat.PopUpSummaryForReport) {
                //detailed results
                var _names = {};
                _cols = [];
                _rows = [];
