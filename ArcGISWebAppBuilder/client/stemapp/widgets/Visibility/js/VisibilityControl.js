  ...
  return dojoDeclare([dijitWidgetBase, dijitTemplatedMixin, dijitWidgetsInTemplate], {
    ...
    postCreate: function () {
      ...
      this.lineStyles = {
        ...
        "esriSLSDashDotDot": "longdashdotdot",
        "esriSLSDot": "dot",
      ...
      if (this.portalUrl) {
        ...
        this.observerHeight.rangeMessage = this.nls.observerRangeMessage;
    ...
    startup: function () {
      ...
      var updateValues = dojoLang.hitch(this, function (a, b, c) {
        ...
        this.tooltip.innerHTML = this.angleUnits.checked ? $("#" + this.FOVInput.id).val() + " " + this.nls.milsLabel
          : c + " " + this.nls.degreesLabel;
      });
    ...
    _syncEvents: function () {
      this.own(
        ...
        dojoOn(this.minObsRange, 'change', dojoLang.hitch(this, this.minObsRangeKeyWasPressed)),
        ...
        dojoOn(this.settingsIcon, "click", dojoLang.hitch(this, function () {
          if (!domClass.contains(this.settingsIcon, "esriCTHidden")) {
            this._showPanel("settingsPage");
          }
        })),

        dojoOn(this.settingsIcon, "keydown", dojoLang.hitch(this, function (evt) {
          if (evt.keyCode === keys.ENTER || evt.keyCode === keys.SPACE) {
            if (!domClass.contains(this.settingsIcon, "esriCTHidden")) {
              this._showPanel("settingsPage");
            }
        ...
        dojoOn(this.settingsPanelBackButton, "click", dojoLang.hitch(this, function () {
          if (!domClass.contains(this.settingsPanelBackButton, "esriCTHidden")) {
            this._SettingsInstance.onSettingsChanged();
            this._showPanel("mainPage");
          }
        })),

        dojoOn(this.settingsPanelBackButton, "keydown", dojoLang.hitch(this, function (evt) {
          if (evt.keyCode === keys.ENTER || evt.keyCode === keys.SPACE) {
            if (!domClass.contains(this.settingsPanelBackButton, "esriCTHidden")) {
              this._SettingsInstance.onSettingsChanged();
              this._showPanel("mainPage");
    ...
    minObsRangeKeyWasPressed: function () {
      if (this.minObsRange.isValid()) {
        this.maxObsRange.constraints.min = Number(this.minObsRange.value) + 0.001;
        this.maxObsRange.set('value', (this.maxObsRange.value &&
          this.maxObsRange.value > this.minObsRange.value) ?
          this.maxObsRange.value : Number(this.minObsRange.value) + 1);
      }
    },
    ...
    distanceUnitDDDidChange: function () {
      ...
      if (this.distanceUnit) {
        ...
        constraints.min = Number(this.minObsRange.value) + 0.001;
      ...
      this._createWedges(params.InputObserver.features, params.MinimumRadius,
        params.MaximumRadius).then(dojoLang.hitch(this, function (wedges) {
          var fullWedgeProjectionDef, wedgeProjectionDef, defList = [],
            geomService, geomServiceUrl;
          if (wedges && wedges.length === 2) {
            //get the geometry service instance
            geomServiceUrl = this.appConfig.geometryService;
            geomService = new GeometryService(geomServiceUrl);
     ...
     * Draw wedge graphics
     */
    _drawWedge: function (graphics, isFullwedge) {
    ...
    _drawViewshed: function (graphics) {
      ...
      nonVisibleAreaGraphics = new Graphic(this._wedgeGeometry);
      nonVisibleAreaGraphics.setAttributes({
        "type": 0,
        "regionType": "nonVisible"
      });
      ...
      if (visibleAreaGeometry) {
        ...
        nonVisibleAreaGraphics = new Graphic(nonVisibleAreaGeometry);
        nonVisibleAreaGraphics.setAttributes({
          "type": 0,
          "regionType": "nonVisible"
        });
    ...
    _showGraphicsOnMap: function (viewShedFeatures) {
      this._drawWedge([new Graphic(this._fullWedgeGeometry)], true);
      this._drawWedge([new Graphic(this._wedgeGeometry)], false);
    ...
    createButtonWasClicked: function () {
      if (this.inputControl.getMapCoordinateDD() && this.minObsRange.isValid() &&
        ...
        parseInt(this.FOVInput.value, 10) !== 0 && !isNaN((parseInt(this.FOVInput.value, 10)))) {
    ...
    _suppoprtAccessibility: function () {
      if (this._currentOpenPanel === "mainPage") {
        utils.initFirstFocusNode(this.parentWidget.domNode, this.settingsIcon);
        if (utils.isAutoFocusFirstNodeWidget(this)) {
          focusUtils.focus(this.settingsIcon);
        }
      ...
      if (this._currentOpenPanel === "settingsPage") {
        utils.initFirstFocusNode(this.parentWidget.domNode, this.settingsPanelBackButton);
    ...
    _getNodeByName: function (panelName) {
      ...
      switch (panelName) {
        ...
        case "publishPage":
          ...
          break;
        case "settingsPage":
          node = this.settingsContainer;
      }
      return node;
    },

    _createRendererForPublishLayer: function (updatedSettings) {
      var visibleSectionFillColor =
        new Color(updatedSettings.visibleSectionFillColor.color);
      var visibleSectionFillTrans =
        (1 - updatedSettings.visibleSectionFillColor.transparency) * 255;
      var visibleSectionOutlineColor =
        new Color(updatedSettings.visibleSectionOutlineColor.color);
      var visibleSectionOutlineTrans =
        (1 - updatedSettings.visibleSectionOutlineColor.transparency) * 255;

      var nonVisibleSectionFillColor =
        new Color(updatedSettings.nonVisibleSectionFillColor.color);
      var nonVisibleSectionFillTrans =
        (1 - updatedSettings.nonVisibleSectionFillColor.transparency) * 255;
      var nonVisibleSectionOutlineColor =
        new Color(updatedSettings.nonVisibleSectionOutlineColor.color);
      var nonVisibleSectionOutlineTrans =
        (1 - updatedSettings.nonVisibleSectionOutlineColor.transparency) * 255;
        ...
        "uniqueValueInfos": [{
          ...
          "symbol": {
            ...
            "outline": {
              ...
              "width": 0.75,
              "type": "esriSLS",
              "style": updatedSettings.visibleSectionOutlineColor.type
            },
            "type": "esriSFS",
            "style": updatedSettings.visibleSectionFillColor.type
          }
        }, {
          "value": 0,
          "symbol": {
            ...
            "outline": {
              ...
              "width": 1,
              "type": "esriSLS",
              "style": updatedSettings.nonVisibleSectionOutlineColor.type
            },
            "type": "esriSFS",
            "style": updatedSettings.nonVisibleSectionFillColor.type
          }
        }]
      };
      // create a renderer for the visibility layer to override default symbology
      this._renderer = new UniqueValueRenderer(uvrJson);
      this._renderer.removeValue("others");
    },

    _createRendererForGraphicsLayer: function (updatedSettings) {
      var visibleSectionFillColor =
        new Color(updatedSettings.visibleSectionFillColor.color);
      var visibleSectionFillTrans =
        (1 - updatedSettings.visibleSectionFillColor.transparency) * 255;
    ...
    _createSettings: function () {
      ...
      this._SettingsInstance = new Settings({
        ...
        config: this.config,
        appConfig: this.appConfig,
        refDomNode: this.parentWidget.domNode
      ...
      this.own(this._SettingsInstance.on("settingsChanged",
        dojoLang.hitch(this, function (updatedSettings) {
          this._createRendererForPublishLayer(updatedSettings);
          this._createRendererForGraphicsLayer(updatedSettings);
        })));
