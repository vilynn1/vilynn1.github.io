...
define([
  ...
  return declare([BaseWidget, _WidgetsInTemplateMixin], {
    ...
    postCreate: function () {
      ...
      this._setAriaLabelToBufferDistTextBox();
    },
    ...
    /**
     * This function is used to set aria-label to buffer distance text box
     * It will be combination of buffer input label, buffer input title
     * current buffer value anf current buffer unit 
     * shapefile.
     * @memberOf Screening/Widget
     */

    _setAriaLabelToBufferDistTextBox: function () {
      domAttr.set(this.bufferDistanceTextBox, "aria-label",
        this.nls.bufferDistanceAndUnit.bufferInputTitle + " " + this.nls.bufferDistanceAndUnit.bufferInputLabel + " " +
        this.bufferDistanceTextBox.getValue() + " " + this.bufferDistanceUnit._getSelectedOptionsAttr().label);
    },
    ...
    onReceiveSearchData: function (data) { // jshint ignore:line
      ...
      if (data && data.source && data.source.locator) {
        ...
        if (domClass.contains(this.placenameButton, "esriCTAOIButtonSelected")) {
          ...
          /**
           * #389 Solved BUG-000122293 The Screening widget in Web AppBuilder for ArcGIS fails to display feature
           * pop-ups for the Place Name tool when both Place Name and Coordinate tools are enabled for the widget.
           * https://github.com/ArcGIS/screening-widget/issues/389
           * Solution: Reopened map infowindow at that searched point location
           */
          setTimeout(lang.hitch(this, function () {
            this.map.infoWindow.show(selectedGraphics[0].geometry);
          }), 500);
        ...
        queryTask.execute(queryObj, lang.hitch(this, function (featureSet) {
          if (featureSet.features) {
            // Check if placename tab is selected or else coordinates tab is selected
            if (domClass.contains(this.placenameButton, "esriCTAOIButtonSelected")) {
              // Search will return only one feature, so add this to graphics in array
               ...
               * pop-ups for the Place Name tool when both Place Name and Coordinate tools are enabled for the widget.
               * https://github.com/ArcGIS/screening-widget/issues/389
               * Solution: Fetched the point from the searched geometry and reopened map infowindow at that point
               */
              var point = this._getPointFromGeometry(selectedGraphics[0].geometry);
              setTimeout(lang.hitch(this, function () {
                this.map.infoWindow.show(point);
              }), 500);
            } else if (domClass.contains(this.coordinatesButton, "esriCTAOIButtonSelected")) {
              //when coordinate tab is selected check for searched geometry type and get point
              //in case of polyline use first point and for polygon use centroid
              if (featureSet.features[0].geometry.type === "point") {
                ...
                this._coordinatesTool.onStartPointSelected(featureSet.features[0].geometry.getPoint(0, 0));
              } else if (featureSet.features[0].geometry.type === "polygon") {
                this._coordinatesTool.onStartPointSelected(featureSet.features[0].geometry.getCentroid());
              }
              this._loadingIndicator.hide();
            }
          } else {
    ...
    /**
     * This function is used to fetch the point from geometry
     * Written to solve the issue https://github.com/ArcGIS/screening-widget/issues/389
     */
      ...
      array.forEach(this.reportDijit._printService.allLayerslegend,
        lang.hitch(this, function (legendLayer) {
          ...
          if (legendLayer.id !== this._aoiGraphicsLayer.id &&
            ...
            /**
             * Solved #387 - BUG-000126938 Legends are visible in the Screening widget report after the
             * 'Hide in Legend' option is enabled in the layer setting.
             * https://github.com/ArcGIS/screening-widget/issues/387
             * If showLegend key is not available it means layer needs to be displayed in the legend
             * If showLegend key is available we need to check for boolean false and do not allow the layer
             * to be added in legendLayers array
             * #387 Solutions starts
             */
            var layerInfo = this.layerInfosObj.getLayerInfoById(legendLayer.id);
            if (layerInfo !== '' && layerInfo !== null && layerInfo !== undefined) {
              if (layerInfo.hasOwnProperty("originOperLayer")) {
                var pushLayer = true;
                if (layerInfo.originOperLayer.hasOwnProperty("showLegend")) {
                  if (!layerInfo.originOperLayer.showLegend) {
                    pushLayer = false;
                  }
                }
                if (pushLayer) {
        ...
        if (data.rows && data.rows.length > 0) {
          ...
          if (data.cols && data.cols.length > 0) {
            ...
            if (aggregatedData.rows && aggregatedData.rows.length > 0) {
              ...
              sortingFieldName = this._getSortFieldValue(this._getSelectedFieldsByLayerId(id),
                this._printData[id].sortInfo.sortingField);
              ...
              if (statisticsFieldsArr.indexOf(sortingFieldName) !== -1) {
                ...
                sortingFieldDisplayText =
                  this._getFieldText(this._getSelectedFieldsByLayerId(id)[sortingFieldName],
                    sortingFieldName);
              ...
              this.sortFieldType = this._getFieldType(id, sortingFieldName, sortingFieldDisplayText);
    ...
    _calculateSelectedUnitAOIArea: function (aoiArea, geometryType, unitDomNode, unitLabel) {
      ...
      domAttr.set(unitDomNode, "aria-label",
        this.nls.reportsTab.featureAreaText + " " + aoiArea + " " + unitLabel);
    },
    ...
    _loadLayoutOptions: function (dropDown) {
      if (this.config.reportSettings.hasOwnProperty("customOptions") &&
        ...
        //Solved issue #380 - Widget cannot be focused using keyboard when
        //reportSettings dosen't have customOptions and defaultPrintOption
        setTimeout(lang.hitch(this, function () {
          this.openAtStartAysn = true; //’this’ is widget object
          this._setAccessibility();
        }), 1500);
    ...
    _setFirstFocusNode: function () {
      if (!domClass.contains(this.aoiLabelContainer, "esriCTHidden")) {
        jimuUtils.initFirstFocusNode(this.domNode, this.aoiLabelContainer);
        return;
      }
      if (!domClass.contains(this.reportLabelContainer, "esriCTHidden")) {
        jimuUtils.initFirstFocusNode(this.domNode, this.reportLabelContainer);
        return;
      }
     ...
     */
    _getFieldType: function (id, sortingFieldName, sortingFieldDisplayText) {
     ...
     */
    _getSortFieldValue: function (configuredFields, selectedSortField) {
      var statisticsFieldsArr = ["esriCTCountField", "esriCTTotalLengthField", "esriCTTotalAreaField"];
      var currentFieldObj, currentFieldText;
      var selectedSortField = selectedSortField;
      var valueArr = Object.keys(configuredFields);
      //for backward
      if (valueArr.indexOf(selectedSortField) === -1 &&
        ...
        for (var fieldName in configuredFields) {
          currentFieldObj = configuredFields[fieldName];
          currentFieldText = this._getFieldText(currentFieldObj, fieldName);
