...
define([
  ...
  var clusterLayer = declare('ClusterLayer', [GraphicsLayer, Evented], {
    ...
    constructor: function (options) {
      ...
      this.staticLayers = options.staticLayers;
    ...
    _initFeatures: function (lyr) {
      ...
      var loading = true;
      var q = new Query();
      if ((this.staticLayers.indexOf(this.lyrType) > -1 || !this.url) && lyr.graphics) {
    ...
    //added for https://devtopia.esri.com/WebGIS/arcgis-webappbuilder/issues/13035
    //The domain is stored with the fields and the GraphicsLayer does not support the concept
    // of fields in the same way a feature layer does.
    //The infoTemplate is still used to handle numeric and date formatting but domains are evaluated here.
    preFormatDomain: function (g) {
      array.forEach(this.fields, lang.hitch(this, function (field) {
        if (field && field.hasOwnProperty("name") && this._parent &&
          g && g.hasOwnProperty("attributes") && g.attributes.hasOwnProperty(field.name)) {
          var domainField = this._parent._checkDomainField(field);
          var subtype = this._parent._checkDomainAndSubtype(this.layerDefinition, field.name, g.attributes,
            { layerObject: this._parentLayer });
          if (domainField || subtype) {
            g.attributes[field.name] = this._parent.formatDomainValue(g.attributes[field.name], domainField, subtype);
          }
        }
      }));
    },
    ...
    isLayerClusterEnabled: function (layerId) {
      var layerFound = false;
      //Loop though all the features to make sure info window has the cluster layer feature
      array.some(this._parent.config.layerInfos, lang.hitch(this,
        function (layerInfo) {
          if (layerId === layerInfo.layer + this._parent.UNIQUE_APPEND_VAL_CL && layerInfo.symbolData.clusteringEnabled) {
            layerFound = true;
            return true;
          }
        }));
    ...
    clusterFeatures: function () {
      ...
      //If the info window is open, then check if at least one selected feature belongs to 
      //cluster layer, is yes then close the info window 
      if (this._map.infoWindow.isShowing) {
        array.some(this._map.infoWindow.features, lang.hitch(this,
          function (feature) {
            if (feature._layer && this.isLayerClusterEnabled(feature._layer.id)) {
              canShowPopup = false;
              return true;
            }
          }));
          //If widget is open and flag is set to false, hide the info window
        if (!canShowPopup && this._parent.isOpen) {
          this._map.infoWindow.hide();
          //If display cluster layer flag is true and popup flag is false 
          //then close the info window
        } else if (this._parent.config.hasOwnProperty("displayClusterLayer") &&
          this._parent.config.displayClusterLayer === true && !canShowPopup) {
          this._map.infoWindow.hide();
        }
      }
