    ...
    return declare([BaseWidgetSetting], {
      ...
      setMaxToleranceForLayer: function (layerId, toleranceSettings) {
        ...
        if (toleranceSettings && toleranceSettings.unit === "px" &&
          ...
          //maxTolerance for this layer is not available OR
      ...
      getDistinctLayers: function (selectedLayerInfo, featureGeometry) {
        ...
        completeDef.then(lang.hitch(this, function () {
            ...
            for (var fieldName in selectedLayerInfo.fieldValues) {
              ...
              for (var i = 0; i < selectedLayerInfo.fieldValues[fieldName].length; i++) {
                ...
                if (copyAction.actionName === "Intersection" && copyAction.enabled) {
                  ...
                  if (copyAction.ignoreLayerRanking) {
                    var closestFeatureInfo = this._getClosestFeature(copyAction.fields);
                    result[fieldName] = closestFeatureInfo.attr;
                    // create layer wise list of fields to be derived from features
                    var fieldsForCopyAction = {};
                    for (var f = 0; f < copyAction.fields.length; f++) {
                      var fInfo = copyAction.fields[f];
                      if (!fieldsForCopyAction.hasOwnProperty(fInfo.layerId)) {
                        fieldsForCopyAction[fInfo.layerId] = [];
                      }
                      if (fieldsForCopyAction[fInfo.layerId].indexOf(fInfo.field) === -1) {
                        fieldsForCopyAction[fInfo.layerId].push(fInfo.field);
                      }
                    }
                    //if multiple values for the field fetch all unique values
                    //skip multiple values when showing layer name in field value
                    if (closestFeatureInfo.intersectingFeatures &&
                      closestFeatureInfo.intersectingFeatures.length > 1) {
                      var values = [];
                      //loop through all the features and create array of unique values for the field
                      array.forEach(closestFeatureInfo.intersectingFeatures, function (feature) {
                        var fieldsToBeUsed = fieldsForCopyAction[feature.layerId];
                        for (var k = 0; k < fieldsToBeUsed.length; k++) {
                          if(feature.attributes && feature.attributes.hasOwnProperty(fieldsToBeUsed[k])){
                            var newValue = feature.attributes[fieldsToBeUsed[k]];
                            if (newValue !== null && newValue !== "" && values.indexOf(newValue) < 0) {
                      ...
                      //show the field in the popup dialog only when multiple unique values found
                      //If we get only one unique value from multiple features, don't show it in the popup
                      if (values.length > 1) {
                        if (!multipleValues) {
                          multipleValues = {};
                        }
                    ...
                    for (var j = 0; j < copyAction.fields.length; j++) {
                      var featureAttributes, featuresInfo, field, layerId, attr;
                      ...
                      if (!result[fieldName].hasOwnProperty(layerId)) {
                        ...
                        if (featuresInfo && featuresInfo.attributes) {
                          featureAttributes = featuresInfo.attributes;
                          ...
                          //if multiple values for the field fetch all unique values
                          //skip multiple values when showing layer name in field value
                          if (featuresInfo.intersectingFeatures &&
                            featuresInfo.intersectingFeatures.length > 1 && field.field !== 'esriCTUseLayerName') {
                            var values = [];
                            //loop through all the features and create array of unique values for the field
                            array.forEach(featuresInfo.intersectingFeatures, function (feature) {
                              var newValue = feature.attributes[field.field];
                              if (newValue !== null && newValue !== "" && values.indexOf(newValue) < 0) {
                                values.push(newValue);
                              }
                            });
          ...
          allDef.resolve({ result: result, multipleValues: multipleValues });
        }));
      ...
      _getIntersectingFeatures: function (layerId, toleranceSettings) {
        ...
        if (this.completeFeatures.hasOwnProperty(layerId)) {
          ...
          //which are intersecting the tolerance settings for this instance of the layers config
          if (allFeatures.length > 0) {
            ...
            if (toleranceValueInMeters) {
              ...
              //so in such case consider all the features as intersecting to geometry
            ...
            if (bufferedGeometry) {
              for (i = 0; i < allFeatures.length; i++) {
                ...
                if (feature.geometry &&
                  ...
                  feature.attributes.esriCTUseLayerName = layerObject.name;
      ...
      _getClosestFeature: function (fields) {
        ...
        returnValue = {};
        fields = this._resetFieldsWithMapLayerOrder(fields);
        ...
        for (j = 0; j < fields.length; j++) {
          ...
          //get intersecting features according to current tolerance settings
        ...
        if (allIntersectingFeatures.length > 0) {
          ...
          //store return value by the closest feature attributes and its layer id
          returnValue[allIntersectingFeatures[0].layerId] = attr;
        }
        return { "attr": returnValue, "intersectingFeatures": allIntersectingFeatures };
      },
      _resetFieldsWithMapLayerOrder: function (fields) {
        var layers, newFieldsArray = [];
        //Get the current map layers
        layers = this._jimuLayerInfos.getLayerInfoArray();
        //map layer order
        array.forEach(layers, lang.hitch(this, function (currentLayer) {
          array.forEach(fields, lang.hitch(this, function (currentField) {
            if (currentLayer.newSubLayers && currentLayer.newSubLayers.length > 0) {
              array.forEach(currentLayer.newSubLayers, lang.hitch(this, function (subLayer) {
                if (subLayer.id === currentField.layerId) {
                  newFieldsArray.push(currentField);
                }
              }));
            } else {
              if (currentLayer.id === currentField.layerId) {
                newFieldsArray.push(currentField);
              }
            }
          }));
      ...
      _getProcessedFeature: function (layerId, toleranceSettings) {
        ...
        return { attributes: featureAttributes, intersectingFeatures: intersectingFeatures };
      },
      ...
      _getIntersectionsForEachField: function (distinctLayers, featureGeometry, completeDef) {
        ...
        //get length of layers which have any tolerance settings applied
        ...
        if (allFieldForSelectedLayer.length > 0) {
          ...
          //if tolerance is defined for any layer, then get the features by async request
      ...
      _getAppropriateGeometryForQuery: function (layerId, featureGeometry) {
        var returnGeometry;
        //if tolerance value is defined for this layer use it otherwise
        ...
        if (this.maxToleranceForlayer.hasOwnProperty(layerId)) {
          ...
          returnGeometry = featureGeometry;
          if (featureGeometry.type === "point") {
            if (this.maxToleranceForlayerInPx.hasOwnProperty(layerId)) {
              returnGeometry = this.pointToExtent(this.map, featureGeometry,
                this.maxToleranceForlayerInPx[layerId]);
            } else {
              returnGeometry = this.pointToExtent(this.map, featureGeometry,
                this.defaultPixelsTolerance);
