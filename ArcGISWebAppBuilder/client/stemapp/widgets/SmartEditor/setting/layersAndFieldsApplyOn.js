...
define(
  ...
  function (
    ...
    return declare([BaseWidgetSetting, Evented, _TemplatedMixin], {
      ...
      searchTextBox: null, //keep the instance of search text box
      defalutFieldInfos: [
        {
      ...
      _addSearchControl: function () {
        ...
        //create node to show switch and label to epand all layers
        var checkAllFieldsNode = domConstruct.create("div", {
          "class": "esriCTExpandAllNode"
        }, this.searchNode);
        ...
        domConstruct.create("label", {
          ...
          "class": "esriCTDojoCheckBoxLabel"
        }, expandAllNode);
        //use dojo checkbox to create switch
        this._checkAllFieldsCheckBox = new dojoCheckBox({
          checked: false
        }, domConstruct.create("div", {}, checkAllFieldsNode));
        //create label to show text "Expand All Layers"
        domConstruct.create("label", {
          "innerHTML": this.nls.actionPage.selectAllLabel,
          "class": "esriCTDojoCheckBoxLabel"
        }, checkAllFieldsNode);
        this.noFieldsDOM = domConstruct.create("div", {
          "innerHTML": this.nls.actionPage.noValidFieldTypeMsg,
          "class": "esriCTHidden"
        }, this.searchNode);
        ...
        //On all nodes checkbox change,
        //Check/Un check all checkboxes based on the value
        this.own(on(this._checkAllFieldsCheckBox, "change", lang.hitch(this, function (isChecked) {
          var fieldsMatchingSearchText, checkboxDOM;
          //If search criteria is specified, get check box nodes which passes the search text
          //If search criteria is EMPTY, get all check box nodes
          if (this.searchTextBox.get("value") !== "") {
            fieldsMatchingSearchText =
              query("div[searchstring*=" + "'" + this.searchTextBox.get("value").toLowerCase() + "']", this.layerAndFieldsMainDiv);
          } else {
            fieldsMatchingSearchText =
              query(".esriCTFieldsDiv", this.layerAndFieldsMainDiv);
          }
          array.forEach(fieldsMatchingSearchText, lang.hitch(this, function (currentNode) {
            checkboxDOM = registry.byNode(query(".esriCTFieldsCheckBox", currentNode)[0]);
            isChecked ? checkboxDOM.check(true) : checkboxDOM.uncheck(true);
          }));
          //Update the table data and update the state of preset value node
      ...
      _searchTextUpdated: function (value) {
        ...
        //Handle the parent check box state once the search text is updated
      ...
      _addLayerAndFields: function () {
            ...
            for (var field in layerDetails[layerId]) {
              ...
              if (layerDetails[layerId][field].editable) {
                ...
                this._createFieldName(layerDetails[layerId][field], layerId,
                  ...
                  matchedFieldValues ? matchedFieldValues.attributeActionGroupName : null);
              }
            }
          }
        }
        ...
        if (hasFields) {
          domClass.add(this.noFieldsDOM, "esriCTHidden");
        } else {
          domClass.remove(this.noFieldsDOM, "esriCTHidden");
        }
      },
       ...
       */
      _createFieldName: function (field, layerId, layerFieldDiv, hasExistingFieldValues, groupName) {
        var checkBox, fieldInfo, message;
        fieldInfo = jimuUtils.getDefaultPortalFieldInfo(field);
        //If groupName is present, use already applied message for group
        //otherwise use the layer level message
        if (groupName) {
          message = this.nls.attributeActionsPage.alreadyAppliedActionMsg;
        } else {
          message = this.nls.attributeActionsPage.alreadyAppliedActionLayerMsg;
        }
        var existingValueMsg = String.substitute(
          message, {
            action: this.nlsActionName[this.actionName]
        });
        //In case of groupName, substitute the group name in the nls
        if (groupName) {
          var groupNameMsg = String.substitute(
            this.nls.attributeActionsPage.appliedGroupName, {
            groupName: groupName
          });
          existingValueMsg = existingValueMsg + " " + groupNameMsg;
        }
        ...
        //Listen for checkbox change and handle the state of parent checkbox
      ...
      /**
       * Handle the state of parent checkbox
       */
      _handleHeaderCheckBoxState: function () {
        var filteredFields, allChecked = true;
        //If search criteria is applied, check only for the filtered rows
        //If value is EMPTY, check for all the rows
        if (this.searchTextBox.get("value") !== "") {
          filteredFields = query("[searchstring*=" + "'" + this.searchTextBox.get("value").toLowerCase() + "']",
            this.layerAndFieldsMainDiv);
        } else {
          filteredFields = query(".esriCTFieldsDiv", this.layerAndFieldsMainDiv);
        }

        if (filteredFields.length > 0) {
          array.some(filteredFields, lang.hitch(this, function (currentNode) {
            var checkBoxDOM = query(".checkbox", currentNode)[0];
            if (!domClass.contains(checkBoxDOM, "jimu-icon-checked")) {
              allChecked = false;
              return true;
            }
          }));
        ...
        if (allChecked) {
          this._checkAllFieldsCheckBox.set("checked", true, false);
        } else {
          this._checkAllFieldsCheckBox.set("checked", false, false);
        }
      },
      ...
      _applyPrevSettings: function () {
        ...
        var allChecked = true;
        var filteredFields = query(".esriCTFieldsDiv", this.layerAndFieldsMainDiv);
        if (filteredFields.length > 0) {
          array.some(filteredFields, lang.hitch(this, function (currentNode) {
            var checkBoxDOM = query(".checkbox", currentNode)[0];
            if (!domClass.contains(checkBoxDOM, "jimu-icon-checked")) {
        ...
        //on popup open set expandAll to true if all fields are selected otherwise set it false
        this._expandAllCheckBox.set("checked", allChecked, false);
      }
    });
  });