define({
  "layersPage": {
    "allLayers": "所有图层",
    "title": "选择一个模板以创建要素，或在地图上单击一个要素以对其进行编辑",
    "generalSettings": "常规设置",
    "layerSettings": "图层设置",
    "webmapSettingsRadioBtnLabel": "支持 web 地图中的设置",
    "customSettingsRadioBtnLabel": "自定义设置",
    "smartActionsTabTitle": "智能操作",
    "attributeActionsTabTitle": "属性操作",
    "geocoderSettingsText": "地理编码器设置",
    "editDescription": "提供要为编辑面板显示的文本",
    "editDescriptionTip": "此文本将显示在模板选取器上方，如果无文本请留空。",
    "promptOnSave": "关闭表单或切换到下一记录时，将提示保存未保存的编辑内容",
    "promptOnSaveTip": "如果当前要素有未保存的编辑内容，将显示提示。 (复制多个要素时会自动保存。)",
    "promptOnDelete": "删除记录时需要确认",
    "promptOnDeleteTip": "当用户单击删除以确认操作时，将显示提示。",
    "removeOnSave": "保存时从选择中移除要素",
    "removeOnSaveTip": "保存记录时，可选择从选择集中移除要素。 如果是唯一选定记录，面板将切换回模板页面。",
    "useFilterEditor": "使用要素模板过滤器",
    "useFilterEditorTip": "过滤模板选取器可用于查看一个图层模板或按名称搜索模板，可选择使用过滤模板选取器。",
    "groupFilteredTemplates": "分组搜索结果",
    "groupFilteredTemplatesTip": "按图层分组搜索结果选项",
    "displayShapeSelector": "显示绘制选项",
    "createNewFeaturesFromExisting": "允许根据现有要素创建新要素",
    "createNewFeaturesFromExistingTip": "允许用户复制现有要素以创建新要素的选项",
    "copiedFeaturesOverrideDefaults": "复制要素中的值会覆盖目标字段中的默认值",
    "copiedFeaturesOverrideDefaultsTip": "复制的要素中的值将仅覆盖匹配字段的默认模板值",
    "displayShapeSelectorTip": "该选项用于显示一系列所选模板的有效绘制选项。",
    "displayPresetTop": "在顶部显示预设值列表",
    "displayPresetTopTip": "该选项用于将预设值列表显示在模板选取器上方。",
    "listenToGroupFilter": "将群组过滤器中的过滤值应用到预设字段",
    "listenToGroupFilterTip": "在群组过滤器微件中应用过滤器后，在预设值列表中向匹配的字段应用值。",
    "keepTemplateActive": "保持所选模板处于活动状态",
    "editLayerAddedFromAddData": "允许编辑从“添加数据”微件添加的图层",
    "editLayerAddedFromAddDataTip": "可以编辑通过“添加数据”微件添加到应用程序中的可编辑图层",
    "keepTemplateActiveTip": "显示模板选取器后，如果之前选择了某一模板，则重新选择该模板。",
    "geometryEditDefault": "默认启用几何编辑",
    "autoSaveEdits": "自动保存新要素",
    "enableAttributeUpdates": "显示属性操作更新按钮",
    "enableAutomaticAttributeUpdates": "更新几何后自动调用属性操作",
    "enableLockingMapNavigation": "启用地图导航锁定",
    "enableMovingSelectedFeatureToGPS": "启用所选点要素到 GPS 位置的移动",
    "enableMovingSelectedFeatureToXY": "启用所选点要素到 XY 位置的移动",
    "featureTemplateLegendLabel": "要素模板和过滤器值设置",
    "saveSettingsLegendLabel": "保存设置",
    "geometrySettingsLegendLabel": "几何设置",
    "actionsSettingsLegendLabel": "操作设置",
    "buttonPositionsLabel": "“保存”、“删除”、“返回”和“关闭”按钮的位置",
    "belowEditLabel": "下方编辑表单",
    "aboveEditLabel": "上方编辑表单",
    "switchToMultilineInput": "如果超出字段长度，则将切换到多行输入",
    "enableValuePicker": "启用相交值选取器",
    "enableValuePickerHint": "如果在相交属性操作中对于一个字段有多个值可用，则将显示提示。",
    "expandRelatedTable": "加载时展开所有相关表格/图层",
    "expandMainTable": "加载时展开所有父图层",
    "copyAndAppendFeatureLabel": "复制和追加要素",
    "allowCopyPolygonLabel": "允许将面复制到点",
    "allowCopyPolygonTooltip": "允许通过几何变换将面要素复制到点目标要素模板的选项",
    "allowCopyLineLabel": "允许将线复制到点",
    "allowCopyLineTooltip": "允许通过几何变换将线要素复制到点目标要素模板的选项",
    "pointAtStartLabel": "始端的点",
    "pointAtStartTooltip": "在要复制的线的起点添加点",
    "pointAtEndLabel": "终点",
    "pointAtEndTooltip": "在要复制的线的终点添加点",
    "layerSettingsTable": {
      "allowDelete": "允许删除",
      "allowDeleteTip": "允许删除 - 选项允许用户删除要素；如果图层不支持删除，则禁用",
      "edit": "可编辑",
      "editTip": "可编辑 - 用于在微件中包括图层的选项",
      "label": "图层",
      "labelTip": "图层 - 地图中定义的图层名称",
      "update": "禁用几何编辑",
      "updateTip": "禁用几何编辑 - 可以选择禁用在放置几何后移动几何或在现有要素上移动几何的功能",
      "allowUpdateOnly": "仅更新版",
      "allowUpdateOnlyTip": "仅更新 - 可以选择仅允许修改现有要素，该选项默认处于选中状态；如果图层不支持创建新要素，则禁用该选项",
      "fieldsTip": "修改要进行编辑的字段并定义智能属性",
      "actionsTip": "操作 - 可以选择编辑字段或访问相关图层/表",
      "description": "描述",
      "descriptionTip": "描述 - 可以输入要在属性页面顶部显示的文本。",
      "relationTip": "查看相关图层和表"
    },
    "editFieldError": "对于不可编辑的图层，字段修改和智能属性不可用",
    "noConfigedLayersError": "智能编辑器需要一个或多个可编辑图层",
    "toleranceErrorMsg": "无效的默认交点容差值",
    "pixelsToleranecErrorMsg": "无效的默认像素容差值",
    "invalidMaxCharacterErrorMsg": "切换到多行输入时的值无效",
    "featuresSelectionTolerance": "设置要素选择的容差（以像素为单位）",
    "featuresSelectionToleranceTooltip": "用户只需单击要素即可轻松打开属性编辑面板",
    "featuresSelectionToleranceErrorMsg": "要素选择容差值无效",
    "autoSaveAttrUpdates": "自动保存属性和几何更新",
    "defaultCoordinateSystem": "默认参考系统",
    "defaultCoordinateSystemTooltip": "设置将所选点要素移动到新位置时的默认参考系统"
  },
  "editDescriptionPage": {
    "title": "为 <b>${layername}</b> 定义属性概览文本 "
  },
  "fieldsPage": {
    "title": "为 <b>${layername}</b> 配置字段",
    "copyActionTip": "属性操作",
    "editActionTip": "智能操作",
    "description": "使用“操作”编辑按钮激活图层上的智能属性。智能属性可以根据其他字段中的值来请求、隐藏或禁用字段。使用“操作”复制按钮并通过交叉口、地址、坐标和预设激活和定义字段值源。",
    "fieldsNotes": "* 是必选字段。如果取消选中显示该字段，并且编辑模板不填充该字段值，您将无法保存新记录。",
    "smartAttachmentText": "为附件配置智能操作",
    "smartAttachmentPopupTitle": "在 <b>${layername}</b> 中为附件配置智能操作",
    "noDisplayFieldWarningMsg": "请至少为图层 ${layerName} 选择一个显示字段",
    "fieldsSettingsTable": {
      "display": "显示",
      "displayTip": "显示 - 确定该字段是否不可见",
      "edit": "可编辑",
      "editTip": "可编辑 - 检查属性表单中是否存在该字段",
      "fieldName": "名称",
      "fieldNameTip": "名称 - 数据库中定义的字段的名称",
      "fieldAlias": "别名",
      "fieldAliasTip": "别名 - 地图中定义的字段的名称",
      "canPresetValue": "预设",
      "canPresetValueTip": "预设 - 此选项可显示预设字段列表中的字段，并允许用户在编辑之前设置值",
      "actions": "操作",
      "actionsTip": "操作 - 更改字段的顺序或设置智能属性"
    },
    "smartAttSupport": "必选数据库字段不支持智能属性"
  },
  "actionPage": {
    "title": "将属性操作配置为 <b>${fieldname}</b>",
    "smartActionTitle": "为 <b>${fieldname}</b> 配置智能操作",
    "description": "指定触发操作的条件之前，这些操作始终处于关闭状态。将按顺序处理操作，并且每个字段仅触发一个操作。可使用“条件编辑”按钮来定义条件。",
    "copyAttributesNote": "禁用具有组名称的任何操作将与独立编辑该操作相同，并且将从相应组中删除该字段的操作。",
    "searchPlaceHolder": "搜索",
    "expandAllLabel": "展开所有图层",
    "selectAllLabel": "选择所有字段",
    "domainListTitle": "属性域字段",
    "noValidFieldTypeMsg": "地图中没有适用于此数据类型的图层",
    "actionsSettingsTable": {
      "rule": "操作",
      "ruleTip": "操作 - 如果满足条件，将执行操作",
      "expression": "表达式",
      "expressionTip": "表达式 - 根据定义的条件以 SQL 格式生成的表达式",
      "groupName": "组名称",
      "groupNameTip": "组名称 - 显示应用表达式的组名称",
      "actions": "条件",
      "actionsTip": "条件 - 触发后，可更改规则的顺序并定义条件"
    },
    "copyAction": {
      "description": "如果已激活字段值源，则会按顺序处理，直到触发有效条件或列表完成为止。 使用“标准编辑”按钮定义标准。",
      "intersection": "交叉口",
      "coordinates": "坐标",
      "address": "地址",
      "preset": "预设",
      "actionText": "操作",
      "criteriaText": "条件",
      "enableText": "启用"
    },
    "actions": {
      "hide": "隐藏",
      "required": "必填",
      "disabled": "已禁用"
    },
    "editOptionsPopup": {
      "editAttributeGroupHint": "警告：独立编辑将从组中移除与此字段关联的选定属性操作",
      "editGroupHint": "警告：独立编辑将从组中移除与此字段关联的选定智能操作",
      "popupTitle": "选择编辑选项",
      "editAttributeGroup": "可从组中定义选定的属性操作。 选择以下选项之一以编辑属性操作：",
      "expression": "可从组中定义选定智能操作的表达式。 选择以下选项之一以编辑智能操作表达式：",
      "editGroupButton": "编辑群组",
      "editIndependentlyButton": "独立编辑"
    }
  },
  "filterPage": {
    "submitHidden": "即使隐藏，仍提交字段的属性数据",
    "title": "为 ${action} 规则配置表达式",
    "filterBuilder": "当记录与以下 ${any_or_all} 表达式相匹配时，设置字段的操作",
    "noFilterTip": "激活操作后，可使用以上工具来定义语句。"
  },
  "geocoderPage": {
    "setGeocoderURL": "设置地理编码器 URL",
    "hintMsg": "请注意：您正在更改地理编码器服务，请务必更新您配置的任何地理编码器字段映射。",
    "invalidUrlTip": "URL ${URL} 无效或不可访问。"
  },
  "addressPage": {
    "popupTitle": "地址",
    "checkboxLabel": "从地理编码器获取值",
    "selectFieldTitle": "属性",
    "geocoderHint": "要更改地理编码器，请转至常规设置中的“地理编码器设置”按钮",
    "prevConfigruedFieldChangedMsg": "在当前地理编码器设置中找不到之前配置的属性。 属性已重置为默认值。",
    "questionMark": "?",
    "consumesCredits": "此工具在与 ArcGIS Online 世界地理编码服务一起使用时会消耗配额。"
  },
  "coordinatesPage": {
    "popupTitle": "坐标",
    "checkboxLabel": "获取坐标",
    "coordinatesSelectTitle": "参考系统",
    "coordinatesAttributeTitle": "属性",
    "mapSpatialReferenceOptionLabel": "地图空间参考",
    "latLongOptionLabel": "纬度/经度",
    "MGRS": "军事格网参考系 (MGRS)",
    "allGroupsCreatedMsg": "已创建所有可能的组",
    "coordinatesSourceTitle": "坐标源",
    "featureLocation": "要素位置",
    "myLocation": "我的位置",
    "LatLong": {
      "y": "纬度",
      "x": "经度",
      "xy": "经度 纬度",
      "yx": "纬度 经度"
    },
    "MapSpatialReference": {
      "y": "Y",
      "x": "X",
      "xy": "XY",
      "yx": "YX"
    }
  },
  "presetPage": {
    "popupTitle": "预设",
    "checkboxLabel": "字段将被预先设置",
    "showOnlyDomainFields": "仅显示域字段",
    "hideInPresetDisplay": "在预设值显示中隐藏",
    "presetValueLabel": "当前预设值为：",
    "changePresetValueHint": "要更改此预设值，请转至常规设置中的“定义预设值”按钮"
  },
  "intersectionPage": {
    "groupNameLabel": "组名称",
    "dataTypeLabel": "数据类型",
    "ignoreLayerRankingCheckboxLabel": "忽略图层排名并在所有已定义图层中查找最接近的要素",
    "intersectingLayersLabel": "要提取值的图层",
    "layerAndFieldsApplyLabel": "要应用提取值的图层和字段",
    "checkboxLabel": "从相交图层的字段获取值",
    "layerText": "图层",
    "fieldText": "字段",
    "actionsText": "操作",
    "toleranceSettingText": "容差设置",
    "addLayerLinkText": "添加图层",
    "useDefaultToleranceText": "使用默认容差",
    "toleranceValueText": "容差值",
    "toleranceUnitText": "容差单位",
    "pixelsUnitText": "像素",
    "useLayerName": "- 使用图层名称 -",
    "noLayersMessage": "无法在地图图层中找到与所选数据类型匹配的字段。",
    "intersectionLayerHint": "将以降序确定图层排序。 使用操作下的箭头对图层重新排序和更改相交优先级。"
  },
  "presetAll": {
    "popupTitle": "定义默认预设值",
    "deleteTitle": "删除预设值。",
    "hintMsg": "下方列出了所有唯一预设字段名称。 移除预设字段的操作将禁用所有图层/表中的相应预设字段。"
  },
  "intersectionTolerance": {
    "intersectionLabel": "所有要素的默认交点容差",
    "intersectionTitle": "启用全局相交容差。",
    "pixelsToleranceLabel": "默认交点容差(像素值)仅适用于点要素",
    "pixelsToleranceTitle": "如果所有要素的默认相交容差设置为 0，则将此设置用于点要素。"
  },
  "smartActionsPage": {
    "smartActionLabel": "配置智能操作",
    "addNewSmartActionLinkText": "新增",
    "definedActions": "定义操作",
    "priorityPopupTitle": "设置智能操作优先级",
    "priorityPopupColumnTitle": "智能操作",
    "priorityOneText": "1",
    "priorityTwoText": "2",
    "priorityThreeText": "3",
    "groupNameLabel": "群组名称",
    "layerForExpressionLabel": "表达式图层",
    "layerForExpressionNote": "注：此图层的字段用于定义表达式条件。 “要应用于的图层和字段”仅显示字段在表达式中定义的图层和表。",
    "expressionText": "表达式",
    "editExpressionLabel": "编辑表达式",
    "layerAndFieldsApplyLabel": "要应用于的图层和字段",
    "submitAttributeText": "即使隐藏，仍提交字段的属性数据",
    "priorityColumnText": "优先",
    "requiredGroupNameMsg": "此值为必填项",
    "uniqueGroupNameMsg": "输入唯一的组名，已经存在具有该名称的组。",
    "deleteGroupPopupTitle": "删除智能操作组",
    "deleteGroupPopupMsg": "删除此组将导致从所有关联的字段中移除此表达式。",
    "invalidExpression": "表达式不能为空",
    "warningMsgOnLayerChange": "定义的表达式及应用的字段将被清除。",
    "smartActionsTable": {
      "name": "名称",
      "expression": "表达式",
      "definedFor": "定义为"
    }
  },
  "attributeActionsPage": {
    "name": "名称",
    "type": "类型",
    "deleteGroupPopupTitle": "删除属性操作组",
    "deleteGroupPopupMsg": "删除此组将导致从所有关联的字段中移除属性操作。",
    "alreadyAppliedActionMsg": "已在组中的此字段上应用 ${action} 操作",
    "alreadyAppliedActionLayerMsg": "已在图层设置的此字段上应用 ${action} 操作",
    "appliedGroupName": "${groupName}",
    "noPresetFieldsMsg": "该组没有相关字段。",
    "presetNoFieldWarningMsg": "请至少为预设组 ${groupName} 配置一个字段"
  },
  "chooseFromLayer": {
    "fieldLabel": "字段",
    "valueLabel": "值",
    "selectValueLabel": "选择值",
    "domainFieldHintLabel": "所选值为编码值属性域。 值 - 将使用 ${domainValue}。"
  },
  "presetPopup": {
    "presetValueLAbel": "预设值"
  },
  "dataType": {
    "esriFieldTypeString": "字符串",
    "esriFieldTypeInteger": "数值",
    "esriFieldTypeDate": "日期",
    "esriFieldTypeGUID": "GUID"
  },
  "relativeDates": {
    "dateTypeLabel": "日期类型",
    "valueLabel": "值",
    "fixed": "固定",
    "current": "当前",
    "past": "过去",
    "future": "未来",
    "popupTitle": "选择值",
    "hintForFixedDateType": "提示：指定的日期和时间将用作预设默认值",
    "hintForCurrentDateType": "提示：当前日期和时间将用作预设默认值",
    "hintForPastDateType": "提示：将从当前日期和时间减去指定值，以获取预设默认值。",
    "hintForFutureDateType": "提示：将当前日期和时间加上指定值，以获取预设默认值。",
    "noDateDefinedTooltip": "未定义日期",
    "relativeDateWarning": "必须指定日期或时间值才能保存默认预设值。",
    "customLabel": "自定义安装"
  },
  "relativeDomains": {
    "fieldSetTitle": "列表",
    "valueText": "值",
    "defaultText": "默认",
    "selectedDomainFieldsHint": "所选属性域字段：${domainFields}",
    "selectDefaultDomainMsg": "请选择默认值域或确保选中所选默认域复选框"
  },
  "valueProviderHint": "注：如果用户不具有所需的权限，则可供选择的值可能不可用。"
});