    ...
    return declare([BaseWidgetSetting, Evented, _TemplatedMixin], {
      ...
      _showFilter: function (tr) {
        ...
        this._filterPage = new FilterPage({
          ...
          _layerDefinition: selectedLayer
        });
      ...
      _createHeaderObj: function (header, tr, index) {
        var headerIconDiv, actionArray = ["", "Hide", "Required", "Disabled"];;
        ...
        //Add header checkbox for all the actions
        if (index <= 3) {
          domClass.add(priorityNumberDiv, "esriCTSAHeaderCheckbox")
          var headerActionCheckBox, actionRow, actionCheckBox;
          headerActionCheckBox = new CheckBox({
            'label': ""
          }, domConstruct.create("div", {}, priorityNumberDiv));
          //Set attribute for further processing
          domAttr.set(headerActionCheckBox.domNode, "action", actionArray[index]);
          //On change check/un check all the checkboxes
          on(headerActionCheckBox, "change", lang.hitch(this, function (isChecked) {
            var currentAction = domAttr.get(headerActionCheckBox.domNode, "action");
            //If search criteria is specified, get the filtered rows
            //If search criteria is not specified, get all the rows
            if (this.searchTextBox.get('value') !== "") {
              actionRow = query("tr[searchstring^=" + "'" +
                this.searchTextBox.get('value').toLowerCase() + "']", this.tableParentContainer);
            } else {
              actionRow = query("tr.esriCTSmartActionGroupRow", this.tableParentContainer);
            }
            //Loop through all the rows and get action checkboxes and accordingly set the state 
            array.forEach(actionRow, lang.hitch(this, function (currentNode) {
              var layerId, field;
              layerId = domAttr.get(currentNode, "layerid");
              field = domAttr.get(currentNode, "field");
              if (layerId && field) {
                this.appliedOn[layerId][field][currentAction] = isChecked;
              }
              actionCheckBox = query("[action=" + currentAction + "]", currentNode);
              if (actionCheckBox && actionCheckBox[0]) {
                actionCheckBox = registry.byNode(actionCheckBox[0]);
                if (isChecked) {
                  actionCheckBox.check(true);
                } else {
                  actionCheckBox.uncheck(true);
                }
              }
            }));
          }));
        ...
        //add search contorl when index is 0
        if (index === 0) {
          this._addSearchControl(priorityColumnParentContainer);
        } else {
      ...
      _addSearchControl: function (searchNode) {
        //Create Textbox to enter GroupName
        this.searchTextBox = new TextBox({
          "trim": true,
          "placeHolder": this.nls.actionPage.searchPlaceHolder,
          "intermediateChanges":true
        }, domConstruct.create("div", {}, searchNode));
        domStyle.set(this.searchTextBox.domNode, "width", "350px");
        this.own(on(this.searchTextBox, "change", lang.hitch(this, this._searchTextUpdated)));
      ...
      _searchTextUpdated: function (value) {
        ...
        if (searchValue !== "") {
          allFields = query("[searchstring]", this.tableParentContainer);
          fieldsMatchingSearchText =
            query("tr[searchstring^=" + "'" + searchValue + "']", this.tableParentContainer);
          //hide all fields
          ...
          layerDetails = this.appliedOn;
          //Handle the parent check box state once the search criteria is updated
          this._handleParentCheckBoxState(fieldsMatchingSearchText);
          for (var layerId in layerDetails) {
            ...
            if (Object.keys(layerDetails[layerId]).length > 0) {
              ...
              fieldsForLayer =
                query("[layerid='" + layerId + "']", this.tableParentContainer);
          ...
          query("[layermaindivid]", this.tableParentContainer).style("display", "");
          query("[searchstring]", this.tableParentContainer).style("display", "");
          query("[searchstring]", this.tableParentContainer).removeClass("esriCTFilteredBySearch");
          query("[searchstring]", this.tableParentContainer).addClass("esriCTNotFilteredBySearch");
          //Handle the parent check box state once the search criteria is updated
          this._handleParentCheckBoxState(fieldsMatchingSearchText);
        }
      },

      _handleParentCheckBoxState: function (fieldsMatchingSearchText) {
        var hideAction = true, requireAction = true, disableAction = true, hideCheckBox,
          requiredCheckBox, disabledCheckBox, hideCheckBoxParent, requiredCheckBoxParent,
          disabledCheckBoxParent;
        if (!fieldsMatchingSearchText) {
          fieldsMatchingSearchText =
            query("tr.esriCTSmartActionGroupRow", this.tableParentContainer);
        }
        //Get all the checkboxes based on the search criteria
        array.forEach(fieldsMatchingSearchText, lang.hitch(this, function (currentNode) {
          hideCheckBox = query("[action='Hide']", currentNode);
          requiredCheckBox = query("[action='Required']", currentNode);
          disabledCheckBox = query("[action='Disabled']", currentNode);
          if (hideCheckBox.length > 0 && requiredCheckBox.length > 0 && disabledCheckBox.length > 0) {
            hideCheckBox = registry.byNode(hideCheckBox[0]);
            requiredCheckBox = registry.byNode(requiredCheckBox[0]);
            disabledCheckBox = registry.byNode(disabledCheckBox[0]);
            //Check the current state of all checkboxes
            //Accordingly keep the flag which will have the information about checkbox check/un check
            if (!hideCheckBox.getValue()) {
              hideAction = false;
            }
            if (!requiredCheckBox.getValue()) {
              requireAction = false;
            }
            if (!disabledCheckBox.getValue()) {
              disableAction = false;
            }
          }
        }));
        //Get the parent checkboxes for all the actions
        hideCheckBoxParent = registry.byNode(query(".esriCTTableTH [action='Hide']", this.tableParentContainer)[0]);
        requiredCheckBoxParent = registry.byNode(query(".esriCTTableTH [action='Required']", this.tableParentContainer)[0]);
        disabledCheckBoxParent = registry.byNode(query(".esriCTTableTH [action='Disabled']", this.tableParentContainer)[0]);
        //Based on the child checkboxes, set the parent checkbox state
        //Hide action
        if (hideAction) {
          hideCheckBoxParent.check(true);
        } else {
          hideCheckBoxParent.uncheck(true);
        }
        //Required action
        if (requireAction) {
          requiredCheckBoxParent.check(true);
        } else {
          requiredCheckBoxParent.uncheck(true);
        }
        //Disabled action
        if (disableAction) {
          disabledCheckBoxParent.check(true);
        } else {
          disabledCheckBoxParent.uncheck(true);
        }
      },

      _createHeaders: function (table) {
        var tr, headerDetails, headerDiv;
        headerDetails = [
          {
            "title": "",
            "icon": "",
      ...
      _createPriorityIcons: function (fieldRow, layerFieldDetails) {
        ...
        var priorityTd = domConstruct.create("td", {style:{"width":"20%"}}, fieldRow);
      ...
      _createLayerName: function (layerRow, layerId) {
        ...
        var layerNameMainDiv = domConstruct.create('div', {'class':'esriCTLayerNameRowParent'}, layerNameTd);
        ...
        this.own(on(layerIconDiv, "click", lang.hitch(this, function (evt) {
          ...
          query('tr[layerid="' + rootLayerId + '"]').toggleClass("esriCTHidden");
          array.some(allLayerIcons, lang.hitch(this, function (layerToggleIcon) {
            //even one layer is not expaned then turn off expandAllCheckBox
            if (domClass.contains(layerToggleIcon, "esriCTToggleLayerExpanded")) {
              enableParent = false;
              return true;
            }
          }));
          this._expandAllCheckBox.set('checked', enableParent, false);
        })));
      ...
      _createFieldNameTd: function (fieldRow, fieldLabel) {
        ...
        domAttr.set(fieldRow, "searchstring", fieldLabel.toLowerCase());
      },
      ...
      _addRows: function (table, details) {
        ...
        for (var layerId in layerDetails) {
          ...
          var layerRow = domConstruct.create('tr',
            {
              "class": "esriCTSmartActionGroupRow esriCTLayerNameRow"
            });
          // layer name
          this._createLayerName(layerRow, layerId);
          domAttr.set(layerRow, "layermaindivid", layerId);
          table.appendChild(layerRow);

          for (var field in layerDetail) {
            var fieldRow = domConstruct.create('tr',
              {
                "class": "esriCTSmartActionGroupRow esriCTLayerFieldRow esriCTHidden"
              });
        ...
        var layerIds = Object.keys(layerDetails);
        //When only one layer is listed, it should be expanded on load.
        if (layerIds && layerIds.length === 1) {
          setTimeout(lang.hitch(this, function () {
            this._expandAllCheckBox.set('checked', true, true);
          }), 100);
        }
        if (layerToExpand.length > 0) {
          setTimeout(lang.hitch(this, function () {
            array.forEach(layerToExpand, function (rootLayerId) {
              ...
              query('div[rootnodelayerid="' + rootLayerId + '"]').toggleClass("esriCTToggleLayerExpanded");
      ...
      _createCheckBox: function (fieldRow, checkBoxInfo, checked, haveExistingExpressions) {
        var checkBoxTD = domConstruct.create('td', { style: { width: "13%" } });
        var checkBoxStatus = true, actionCheckBox, actionCheckBoxParent;
        ...
        domAttr.set(checkBoxDiv, "field", checkBoxInfo.field);
        domAttr.set(checkBoxDiv, "layerid", checkBoxInfo.layerid);
        ...
        this.own(on(checkBox, "change", lang.hitch(this, function (checked) {
          var action = domAttr.get(checkBoxDiv, "action");
          checkBoxStatus = true;
          appliedOn[action] = checked;
          if (this.searchTextBox.get("value") !== "") {
            //Get the dom node based on specified search criteria
            fieldsMatchingNodes =
              query("tr[searchstring^=" + "'" + this.searchTextBox.get("value").toLowerCase() + "']",
                this.tableParentContainer);
          } else {
            fieldsMatchingNodes = query("tr.esriCTSmartActionGroupRow", this.tableParentContainer);
          }
          if (fieldsMatchingNodes.length > 0) {
            //Loop through all the nodes for a particular action
            //Accordingly keep the flag which will have the status of checkbox
            array.some(fieldsMatchingNodes, lang.hitch(this, function (currentNode) {
              var actionChkBoxDOM = query("[action=" + action + "]", currentNode);
              if (actionChkBoxDOM && actionChkBoxDOM[0]) {
                actionCheckBox = registry.byNode(actionChkBoxDOM[0]);
                if (actionCheckBox && !actionCheckBox.getValue()) {
            ...
            //If the current search string returns 0 rows
            //Update the checkbox status to uncheck
            checkBoxStatus = false;
          }
          //Get the header checkbox for a particular action
          //Set the parent checkbox state based on wether all the child checkboxes are checked/un checked
          actionCheckBoxParent = registry.byNode(query(".esriCTTableTH [action=" + action + "]", this.tableParentContainer)[0]);
          if (checkBoxStatus && !actionCheckBoxParent.getValue()) {
            actionCheckBoxParent.check(true);
          } else {
            actionCheckBoxParent.uncheck(true);
          }
        })));
      ...
      _createTable: function (appliedOn) {
        ...
        domClass.add(table, "esriCTTableMainNode tableBodyScroll");
        this._createHeaders(table);
        this.tableParentContainer.appendChild(table);
        var bodyDiv = domConstruct.create('tbody', {
          "style": "width: 808px"
        });
          ...
          if (!layer.isTable || (layer.isTable && layer.layerObject.relationships.length > 0)) {
            ...
            if (configuredLayerInfo && configuredLayerInfo.fieldInfos &&
              ...
              array.forEach(configuredLayerInfo.fieldInfos, function (field) {
                var prioritiesInfo;
                var portalFieldInfo = jimuUtils.getDefaultPortalFieldInfo(field);
                ...
                if (field.type !== "esriFieldTypeGeometry" &&
                  ...
                  fieldLabels[layer.id][field.name] = portalFieldInfo.label;
