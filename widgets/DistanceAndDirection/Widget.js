...
define([
  ...
  var clz = dojoDeclare([jimuBaseWidget, dijitWidgetsInTemplate], {
    ...
    postCreate: function () {
      ...
      this._layerList = this._lineLayerList.concat(this._polygonLayerList);
      ...
      this.addDDNameArea.missingMessage = this.nls.missingLayerNameMessage;
    ...
    resize: function () {
      ...
      var nl = dojoQuery(".dijitValidationTextBoxLabel", this.featureLayerList.domNode);
      if (nl.length > 0) {
        domStyle.set(nl[0], "width", (this.ddPublishButton.clientWidth - 44) + "px");
      }
    },
    ...
    _ddPublishButtonClicked: function () {
      ...
      var params = {
        layerName: (this.publishNewLayer.checked) ? this.addDDNameArea.value : this.featureLayerList.getValue(),
        url: "",
        itemId: "",
        serviceItemId: ""
      };
    ...
    _initSaveToPortal: function (params) {
      ...
      portal.signIn().then(lang.hitch(this, function (portalUser) {
        ...
        if (!params.url) {
          ...
          portalutils.isNameAvailable(checkServiceNameUrl, token,
            ...
            if (response0.available) {
              ...
              portalutils.createFeatureService(createServiceUrl, token,
                    ...
                    function (response2) {
                      if (response2.success) {
                        ...
                        newFeatureLayer.applyEdits(newGraphics, null, null).then(lang.hitch(this, function () {
                          ...
                          this._setFocus();
                          this._reset();
                        })).otherwise(lang.hitch(this, function (editsErr) {
                          this.publishMessage.innerHTML = editsErr.message;
                          this._setFocus();
                          this.busyIndicator.hide();
                          this._reset();
                        }));
                ...
                this.busyIndicator.hide();
                this.publishMessage.innerHTML =
                  jimuUtils.sanitizeHTML(this.nls.createService.format(err1.message));
                this._setFocus();
              }));
            } else {
          ...
          var newGraphics = [];
          this._graphicsAdded.forEach(function (g) {
            if (g.attributes !== undefined && !g.attributes.hasOwnProperty("isDirectionalGraphic")) {
              //store all the attributes in case when creating new layer
              //only filter those internal attributes created previously.
              //for e.g. GeoLength/LineAngle in case of TabLine
          ...
          newFeatureLayer.applyEdits(newGraphics, null, null).then(lang.hitch(this, function () {
            ...
            this._reset();
            this.busyIndicator.hide();
          })).otherwise(lang.hitch(this, function (applyEditsErr) {
            this.publishMessage.innerHTML = applyEditsErr.message;
            this._setFocus();
            this.busyIndicator.hide();
            this._reset();
          }));
    ...
    _populateSelectList: function (selectNode, layerList, selectedOptionName) {
      ...
      if (selectNode.options.length === 0) {
        ...
        domClass.add(this.checkBoxParentContainer, "DDHidden");
      } else {
        domClass.remove(this.checkBoxParentContainer, "DDHidden");
      }
